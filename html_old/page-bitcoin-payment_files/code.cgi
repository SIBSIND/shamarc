//scgi 
//cached 
 
		
var IKLAD_FUNC = {
	get_cookie: function ( check_name ) {
		var a_all_cookies = document.cookie.split( ';' );
		var a_temp_cookie = '';
		var cookie_name = '';
		var cookie_value = '';
		var b_cookie_found = false;
	
		for ( i = 0; i < a_all_cookies.length; i++ )	{
			a_temp_cookie = a_all_cookies[i].split( '=' );
			cookie_name = a_temp_cookie[0].replace(/^\s+|\s+$/g, '');
			if ( cookie_name == check_name ){
				b_cookie_found = true;
				if ( a_temp_cookie.length > 1 ){
					cookie_value = unescape( a_temp_cookie[1].replace(/^\s+|\s+$/g, '') );
				};
				return cookie_value;
				break;
			};
			a_temp_cookie = null;
			cookie_name = '';
		}
		if ( !b_cookie_found )	{
			return null;
		}
	},

	set_cookie: function ( name, value, expires_min, path, domain, secure ) {
		var today = new Date();
		today.setTime( today.getTime() );
		
		if ( expires_min ) {
			expires_min = expires_min * 1000 * 60;
		};
		var expires_date = new Date( today.getTime() + (expires_min) );
		
		document.cookie = name + "=" + escape( value ) +
		( ( expires_min ) ? ";expires=" + expires_date.toGMTString() : "" ) +
		( ( path ) ? ";path=" + path : "" ) +
		( ( domain ) ? ";domain=" + domain : "" ) +
		( ( secure ) ? ";secure" : "" );
	},
	
	delete_cookie: function ( name, path, domain ) {
		if ( IKLAD_FUNC.get_cookie( name ) ) {
			document.cookie = name + "=" + ( ( path ) ? ";path=" + path : "") + 
			( ( domain ) ? ";domain=" + domain : "" ) + 
			";expires=Thu, 01-Jan-1970 00:00:01 GMT";
		};
	},
	
	object_to_string: function (obj) {
		var has_NL = false;
		var NL_SPL = '<NL>';
		
		for (var v in obj) {
			if (obj[v] == null) {
				obj[v] = '';
			};
		};
		
		for (var v in obj) {
			if (obj[v].toString().indexOf("\n") >= 0) {
				has_NL = true;
				break;
			};
		};
		
		if (has_NL) {
			while (1) {
				NL_SPL += Math.floor(Math.random() * 10);
				var NL_found = false;
				
				for (var v in obj) {
					if (obj[v].toString().indexOf(NL_SPL) >= 0) {
						NL_found = true;
						break;
					};
				};
				
				if (NL_found) {
					continue;
				} else {
					break;
				};
			};
			
			
			for (var v in obj) {
				obj[v] = obj[v].toString().replace(/\n/g, NL_SPL);
			};
			obj._NL = NL_SPL;
		};
		
		
		var s = '';
		for (var v in obj) {
			s += v + ':' + obj[v].toString() + "\n";
		};
		
		return s;
	},
	
	string_to_object: function (str) {
		var obj = new Object;
		
		var lines = str.split("\n");
		
		for (var i=0; i < lines.length; i++) {
			var line = lines[i];
			
			if (line.match(/^(.*?):((.|\r)*)$/)) {
				obj[RegExp.$1] = RegExp.$2;
			};
		};
		
		if (typeof obj._NL != 'undefined') {
			var rx = new RegExp(obj._NL, 'g');
			for (var v in obj) {
				obj[v] = obj[v].toString().replace(rx, "\n");
			};
		};
		
		delete obj._NL;
		return obj;
	},

	find_object_position: function (obj) {
	  var curleft = 0;
	  var curtop = 0;
	   
	  if (obj.offsetParent) {
	    while(1)  {
	      curleft += obj.offsetLeft;
	      curtop += obj.offsetTop;
	      obj = obj.offsetParent;
	      if (! obj) { break; };
	    };
	  } else  {
	  	if(obj.x) curleft += obj.x;
	    if(obj.y) curtop += obj.y;
	  };
	  
	  return [curleft, curtop];
	}


};

		
		var IKLAD_2423 = {
			client_id: '',
			iframe: '',
			chat_window_open: 0,
			
			curr_operator_status: '',
			CODE_CONTAINER: '',
			
			SYSTEM: {
				INITIALIZED: false,
				ACC_TYPE: 'p'
			},
			
			TEMPLATE: {
				HTML_CODE: '',
				OPINIONS: [
{
 id: 1,
text: 'Super',
want_comment: 0
},
{
 id: 2,
text: 'Good',
want_comment: 0
},
{
 id: 3,
text: 'Normal',
want_comment: 0
},
{
 id: 4,
text: 'Bad',
want_comment: 1
},
{
 id: 5,
text: 'Worst',
want_comment: 1
}
]
			},
			
			OPERATOR_STATUS: {
				current_status: 'offline'
			},
			
			AUTO_CMD_POLLER: {
				active: false,
				timer: '',
				last_id : 0,
				poll_time: 30000, //default = each 30 seconds
				
				_MSG_DATA: '',
				
				start: function () {
					if (IKLAD_2423.AUTO_CMD_POLLER.active) return;
			
					IKLAD_2423.AUTO_CMD_POLLER.active = true;
					IKLAD_2423.AUTO_CMD_POLLER.timer = setTimeout('IKLAD_2423.AUTO_CMD_POLLER.poll()', 100);
				},
				
				stop: function () {
					IKLAD_2423.AUTO_CMD_POLLER.active = false;
					clearTimeout(IKLAD_2423.AUTO_CMD_POLLER.timer);
					IKLAD_2423.AUTO_CMD_POLLER.timer = '';
				},
				
				poll: function () {
					if (! IKLAD_2423.AUTO_CMD_POLLER.active) return;
					
					var s = document.createElement('script');
					s.type = 'text/javascript';
					s.charset = 'utf-8';
					s.src = 'https://chat.iklad-chat.biz/outchat_cmd.cgi?c=2423&cid=' + IKLAD_2423.client_id + '&aid=' + IKLAD_2423.AUTO_CMD_POLLER.last_id + '&v=2&u=' + escape(window.location.href.slice(7)) + '&r=' + Math.random();
					IKLAD_2423.CODE_CONTAINER.appendChild(s);
				},
				
				
				on_message: function (msg_id, msg_data) {
					if (! IKLAD_2423.AUTO_CMD_POLLER.active) return;	//this can happen, coz script loading can be initiated before chat window opened

					IKLAD_2423.AUTO_CMD_POLLER.last_id = msg_id;
					
					//we should NOT call 'notify_received' here. (it will be called by 'outchat_cmd' script if needed)
					
					//display msg in HINT box
					if ((typeof IKLAD_HINT_2423 == 'object') && (typeof IKLAD_HINT_2423.show == 'function')) {
						var coords_descr = IKLAD_STATUS_2423.get_hint_box_coords();
						
						if (coords_descr) {
							IKLAD_HINT_2423.show(msg_data, coords_descr);
						};
					};
				},
				
				notify_received: function (msg_id, options) {
					options.force = typeof options.force == 'undefined' ? false : options.force;
					options.is_auto = typeof options.is_auto == 'undefined' ? true : options.is_auto;
					
					if (! options.force) {
						if (! IKLAD_2423.AUTO_CMD_POLLER.active) return;
					};
					
					var s = document.createElement('script');
					s.type = 'text/javascript';
					s.charset = 'utf-8';
					s.src = 'https://channel2423.iklad-chat.biz/send/c=2423&id=' + IKLAD_2423.client_id + '&cmd=offchat_msg_received/' + (options.is_auto ? 1 : 0) + ',' + msg_id + '/' + Math.random();
					
					IKLAD_2423.CODE_CONTAINER.appendChild(s);
				}
			},
		
			AUTO_DIE: {
				active: false,
				timer: '',
				died: false,
				
				start: function () {
					if (IKLAD_2423.AUTO_DIE.active) return;
					
					IKLAD_2423.AUTO_DIE.timer = setTimeout('IKLAD_2423.AUTO_DIE.die()', 1000*300);
					IKLAD_2423.AUTO_DIE.active = true;
				},
				
				stop: function () {
					IKLAD_2423.AUTO_DIE.active = false;
					
					clearTimeout(IKLAD_2423.AUTO_DIE.timer);
					IKLAD_2423.AUTO_DIE.timer = null;
					
					
					//if 'stop' called after we 'died' - do UN-die
					if (IKLAD_2423.AUTO_DIE.died) {
						IKLAD_2423.AUTO_DIE.died = false;
						IKLAD_2423.iframe.contentWindow.postMessage('init_wait:' + "\n", '*'); //un-qq!
					};
				},
				
				die: function () {
					//stop communication channel
					IKLAD_2423.iframe.contentWindow.postMessage('de_init_wait:' + "\n", '*'); //un-qq!
					IKLAD_2423.AUTO_DIE.stop();
					
					IKLAD_2423.AUTO_DIE.died = true;
				}
			},
			
			COBROWSE: {
				script_loaded: false,
				queue: [],
				
				load_script: function () {
					var s = document.createElement('script');
					s.type = 'text/javascript';
					s.charset = 'utf-8';
					s.src = 'https://chat.iklad-chat.biz/cobrowse.cgi?c=2423&v=2&cid=' + IKLAD_2423.client_id + '&r=' + Math.random();
					IKLAD_2423.CODE_CONTAINER.appendChild(s);
					
					IKLAD_2423.COBROWSE.script_loaded = true;
				},
				
				add_command_to_queue: function (data) {
					if (! IKLAD_2423.COBROWSE.script_loaded) IKLAD_2423.COBROWSE.load_script();
					
					if (typeof IKLAD_2423.COBROWSE.process_command == 'function') {
						IKLAD_2423.COBROWSE.process_command(data);
					} else {
						IKLAD_2423.COBROWSE.queue.push(data);
					};
				}
			},
		
			generate_client_id: function (in_len) {
				var len = (in_len ? in_len : 30);
				var chars = '_qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM1234567890';
				var res = '';
				
				for (i=0; i <= len; i++) {
					var st = Math.floor(Math.random() * chars.length);
					res += chars.substr(st,1);
				};
				
				return res;
			},
			
			check_IKLAD_list_init_allowed: function () {
				var ks = Object.keys(window);
				
				for (var i=0; i < ks.length; i++) {
					if (ks[i] == 'IKLAD_2423') continue;
					
					if (ks[i].match(/^IKLAD_(\d+)$/)) {
						var other_ch = RegExp.$1;
						
						if (typeof window[ks[i]].SYSTEM == 'undefined') continue;
						if (! window[ks[i]].SYSTEM.INITIALIZED) continue;
						
						if (IKLAD_2423.SYSTEM.ACC_TYPE == 'f') {
							return false;
						} else {
							if (window[ks[i]].SYSTEM.ACC_TYPE == 'f') {
								if (window['IKLAD_' + other_ch.toString()].CODE_CONTAINER) {
									window['IKLAD_' + other_ch.toString()].CODE_CONTAINER.innerHTML = '';
								};
							};
						};
					};
					
				};
				
				return true;
			},
		
			init: function () {
				if (IKLAD_2423.SYSTEM.INITIALIZED) return;
				if (! IKLAD_2423.check_IKLAD_list_init_allowed()) return;
				
				if (typeof IKLAD_2423.check_allowed_domains == 'function') {
					if (! IKLAD_2423.check_allowed_domains()) {
						return;
					};
				};
				
				// make CODE_CONTAINER
				if (! document.getElementById('IKLAD_CONTAINER_2423')) {	//create new if not pasted to code 'inplace'
					var container = document.createElement('div');
					container.setAttribute("id", "IKLAD_CONTAINER_2423");
					document.body.appendChild(container);
				};
				
				IKLAD_2423.CODE_CONTAINER = document.getElementById('IKLAD_CONTAINER_2423');
				
				//install OnMessage handler
				IKLAD_2423.add_listener(window, "message", IKLAD_2423.on_message_handler);
				
								
				//get||generate client_id
				if ((typeof IKLAD_POPUP == 'undefined') || (typeof IKLAD_POPUP.client_id == 'undefined')) {
					IKLAD_2423.client_id = IKLAD_FUNC.get_cookie("sitechat_ID_2423");
					if (! IKLAD_2423.client_id) {
						IKLAD_2423.client_id = IKLAD_2423.generate_client_id();
					};
					IKLAD_FUNC.set_cookie("sitechat_ID_2423", IKLAD_2423.client_id, 525948, '/');	//1 year
				} else {
					IKLAD_2423.client_id = IKLAD_POPUP.client_id;
				};
				
				//append form HTML
				var form_container = document.createElement('div');
				form_container.innerHTML = IKLAD_2423.TEMPLATE.HTML_CODE;
				document.body.appendChild(form_container);
								
				try {
					IKLAD_STATUS_2423.init({'operator_status': IKLAD_2423.OPERATOR_STATUS.current_status});
				} catch (e) {};
					
				try {
					IKLAD_HINT_2423.init();
				} catch (e) {};
				
				try {
					IKLAD_TEMPLATE_2423.init({
						'operator_status': IKLAD_2423.OPERATOR_STATUS.current_status,						
						'opinions': IKLAD_2423.TEMPLATE.OPINIONS
					});
				} catch (e) {};
				
				
				//create communication iframe
				IKLAD_2423.iframe = document.createElement('iframe');
				IKLAD_2423.iframe.style.display = 'none';
				IKLAD_2423.iframe.setAttribute('style', 'display:none;');
				IKLAD_2423.iframe.src = "https://channel2423.iklad-chat.biz/iframe.cgi?c=2423&v=2&clientid=" + IKLAD_2423.client_id + '&ssl=' + 1;// + "&r=" + Math.random(1);
				document.body.appendChild(IKLAD_2423.iframe);
				
				IKLAD_2423.SYSTEM.INITIALIZED = true;
			},
		
		
	    add_listener: function (element, event, listener, bubble) {
	      if(element.addEventListener) {
	        if(typeof(bubble) == "undefined") bubble = false;
	        element.addEventListener(event, listener, bubble);
	        return true;
	      } else if (element.attachEvent) {
	        element.attachEvent("on" + event, listener);
	        return true;
	      };
	      
	      return false;
	    },
		
		
			on_message_handler: function (evt) {				
				if (evt.origin != 'https://channel2423.iklad-chat.biz') return;
				
				if (evt.data == 'iframe_ready') {					
					var state_obj = {
						title: document.title,
						curr_url: decodeURIComponent(window.location.href.slice(7)),
						chat_window_open: 0	//start with 0 always
					};
					
					if (document.referrer) {
						state_obj.referrer = decodeURIComponent(document.referrer.slice(7));
					};
					
					//send client_state (if we are NOT in popup)
					if (typeof IKLAD_POPUP == 'undefined') {
						evt.source.postMessage('client_state_data:' + "\n" + JSON.stringify(state_obj), evt.origin);
					};
					
					evt.source.postMessage("init_wait:\n", evt.origin);	//init_wait
					
					
					if (typeof IKLAD_POPUP == 'undefined') {
						var WindowOpenStatus = IKLAD_FUNC.get_cookie("IKLAD.window_open_2423");
						if (WindowOpenStatus && (WindowOpenStatus == 1)) {	//reopen window
							IKLAD_TEMPLATE_2423.open_chat_window();
						} else {
							IKLAD_2423.AUTO_CMD_POLLER.start();
							IKLAD_2423.AUTO_DIE.start();
						};
					} else {
						IKLAD_TEMPLATE_2423.open_chat_window();
					};
								
					return;
				};
				
				
				if (evt.data.match(/get_form_data_reply:\n((.|[\n\r])*)/)) {
					var data = JSON.parse(RegExp.$1);
					if (IKLAD_TEMPLATE_2423.form_data) IKLAD_TEMPLATE_2423.form_data(data);
					
					return;
				};
				
				
				if (evt.data.match(/send_form_data_result:\n((.|[\n\r])*)/)) {
					var result = JSON.parse(RegExp.$1);
					if (IKLAD_TEMPLATE_2423.send_form_data_result) IKLAD_TEMPLATE_2423.send_form_data_result(result);
					
					return;
				};
				
				
				if (evt.data.match(/operator_message:\n((.|[\n\r])*)/)) {
					var msg_obj = JSON.parse(RegExp.$1);
					
					IKLAD_TEMPLATE_2423.on_chat_message(msg_obj);
						
					if (IKLAD_2423.chat_window_open == '0') {	//closed chat window
						if ((msg_obj.to == 'client') && (msg_obj.by_human == '1')) {
							IKLAD_2423.AUTO_CMD_POLLER.notify_received(msg_obj.id, {is_auto: false});
							
							//display msg in HINT box
							if ((typeof IKLAD_HINT_2423 == 'object') && (typeof IKLAD_HINT_2423.show == 'function')) {
								var coords_descr = IKLAD_STATUS_2423.get_hint_box_coords();
								
								if (coords_descr) {
									IKLAD_HINT_2423.show(msg_obj.text, coords_descr);
								};
							};
						};
					};
					
					return;
				};
				
				
				if (evt.data.match(/full_chat_history:\n((.|[\n\r])*)/)) {	
					var msgs_arr = JSON.parse(RegExp.$1);
					IKLAD_TEMPLATE_2423.full_chat_log_data({'msgs': msgs_arr});
					return;
				};
				
				
				if (evt.data.match(/operator_status:\n((.|[\n\r])*)/)) {
					var event_obj = JSON.parse(RegExp.$1);

					IKLAD_2423.curr_operator_status = event_obj.status;

					var status_object = {
						'operator_status': event_obj.status
					};
											
					if (event_obj.status == 'offline') {
						status_object.actions = event_obj.actions;
					};

					if (event_obj.status == 'error') {
						status_object.error_message = event_obj.error_message;
					};

					//notify template(s)
					IKLAD_STATUS_2423.operator_status_changed(status_object);
					IKLAD_TEMPLATE_2423.operator_status_changed(status_object);

					//call api
					if (typeof IKLAD_API_2423 == 'object') {
						
						if ( (event_obj.status == 'online') && (typeof IKLAD_API_2423.onReady_func == 'function') ) {	//this will be true only once per page load
							IKLAD_API_2423.onReady_func();
							IKLAD_API_2423.onReady(undefined);	//reset to undef
						};
						
						if ( (event_obj.status == 'offline') && (typeof IKLAD_API_2423.onReadyOffline_func == 'function') ) {	//this will be true only once per page load
							IKLAD_API_2423.onReadyOffline_func();
							IKLAD_API_2423.onReadyOffline(undefined);	//reset to undef
						};
						
					};
					
					return;
				};
				
				if (evt.data.match(/operator_data:\n((.|[\n\r])*)/)) {
					var event_obj = JSON.parse(RegExp.$1);
					IKLAD_TEMPLATE_2423.operator_data_changed(event_obj);
					return;
				};
				
				if (evt.data.match(/operator_typing:\n((.|[\n\r])*)/)) {
					var event_obj = JSON.parse(RegExp.$1);
					IKLAD_TEMPLATE_2423.operator_activity(event_obj);
					return;
				};
				
				if (evt.data.match(/operator_command:\n((.|[\n\r])*)/)) {
					var event_obj = JSON.parse(RegExp.$1);
				
					if (event_obj.command == 'goto') {
						if (typeof IKLAD_POPUP == 'undefined') {
							window.location.href = event_obj.url;
						} else {
							try {
								window.opener.location.href = event_obj.url;
							} catch (e) {};
						};
					};
					
					if (typeof IKLAD_POPUP != 'undefined') {
						return;	//do not process NEXT commands in popup state
					};
					
					if (event_obj.command == 'openchat') {
						IKLAD_TEMPLATE_2423.open_chat_window();
					};
					
					if (event_obj.command == 'closechat') {
						IKLAD_TEMPLATE_2423.close_chat_window();
					};
											
					if (event_obj.command == 'cobrowse_cmd') {
						IKLAD_2423.COBROWSE.add_command_to_queue(event_obj.data);
					};
					
					return;
				};
				
				
				if (evt.data.match(/jseval:\n((.|[\n\r])*)/)) {
					var event_obj = JSON.parse(RegExp.$1);
					try { eval(event_obj.code); } catch (e) {};
					return;
				};
				
			},
			
			
			
			
			//callbacks from TEMPATE
			notify_window_state_changed: function (params) {
				//params.state: 'open' || 'closed' ( window would not KNOW it is in 'popup' state)

				var this_state = params.state;
				
				if ((this_state == 'open') && (typeof IKLAD_POPUP != 'undefined')) {
					this_state = 'popup';
				};
				
				if (this_state == 'open') {
					IKLAD_2423.chat_window_open = 1;
				};
				
				if (this_state == 'popup') {
					IKLAD_2423.chat_window_open = 2;
				};
				
				if (this_state == 'open' || this_state == 'popup') {
					IKLAD_FUNC.set_cookie("IKLAD.window_open_2423", 1, 30, '/');
					IKLAD_2423.AUTO_CMD_POLLER.stop();
					IKLAD_2423.AUTO_DIE.stop();
					
					//call api
					if ((typeof IKLAD_API_2423 == 'object') && (typeof IKLAD_API_2423.onChatWindowOpen_func == 'function')) {
						IKLAD_API_2423.onChatWindowOpen_func();
					};
				};
				
				
				if (this_state == 'closed') {
					IKLAD_2423.chat_window_open = 0;
					IKLAD_FUNC.delete_cookie("IKLAD.window_open_2423", '/');
					IKLAD_2423.AUTO_CMD_POLLER.start();
					IKLAD_2423.AUTO_DIE.start();
					
					//call api
					if ((typeof IKLAD_API_2423 == 'object') && (typeof IKLAD_API_2423.onChatWindowClose_func == 'function')) {
						IKLAD_API_2423.onChatWindowClose_func();
					};
				};
				
				IKLAD_STATUS_2423.window_state_changed({'state': this_state});
				IKLAD_HINT_2423.window_state_changed({'state': this_state});	//will hide hint box on chat window open
				IKLAD_2423.iframe.contentWindow.postMessage('client_state_data:' + "\n" + JSON.stringify({'chat_window_open': IKLAD_2423.chat_window_open}), '*');	//un-qq!
			},
			
			notify_client_activity: function (params) {
				var obj;
				
				if (params.type == 'client_typing') {
					obj = {
						'type': 'client_typing',
						'status': params.status
					};
				};
				
				if (obj) IKLAD_2423.iframe.contentWindow.postMessage('client_activity:' + "\n" + JSON.stringify(obj), '*');
			},
			
			send_message: function (msg_obj) {
				IKLAD_2423.iframe.contentWindow.postMessage('send_message:' + "\n" + JSON.stringify(msg_obj), '*');
			},
			
			play_sound: function (type) {
				IKLAD_2423.iframe.contentWindow.postMessage('play_sound:' + "\n" + JSON.stringify({'type': type}), '*');
			},
			
			send_opinion: function (id, comment) {
				IKLAD_2423.iframe.contentWindow.postMessage('send_opinion:' + "\n" + JSON.stringify({'id': id, 'comment': comment}), '*');
			},
			
			send_chat_log_to_email: function (email) {
				IKLAD_2423.iframe.contentWindow.postMessage('send_chat_log_to_email:' + "\n" + JSON.stringify({'email': email}), '*');
			},
			
			//user want open chat in new (external) window
			// 2 parameters is REQUIRED:
			//params.height - height of new window
			//params.width - width of new window
			open_in_new_window: function (params) {
				
				var New_win =
				  window.open("https://chat.iklad-chat.biz/ext_win_chat.cgi?c=2423&clientid=" + IKLAD_2423.client_id + "&rnd=" + Math.random(), 
				   "IKLAD_PopupWindow_2423",
				   "menubar=no,location=no,resizable=no,scrollbars=no,status=no,directories=no,height=" + params.height + ",width=" + params.width
				 );
				 
			},
			
			send_offline_form: function (params) {
				IKLAD_2423.iframe.contentWindow.postMessage('send_offline_form:' + "\n" + JSON.stringify(params), '*');
			},
			
			
			get_form_data: function (form_id, msg_id) {
				var params = {
					"form_id": form_id, 
					"msg_id": msg_id
				};
				
				IKLAD_2423.iframe.contentWindow.postMessage('get_form_data:' + "\n" + JSON.stringify(params), '*');
			},
			
			
			send_form_data: function (form_id, msg_id, form_data) {
				var params = {
					"form_id": form_id, 
					"msg_id": msg_id,
					"form_data": form_data
				};
				
				IKLAD_2423.iframe.contentWindow.postMessage('send_form_data:' + "\n" + JSON.stringify(params), '*');
			},
			
			notify_form_completed: function (form_id, msg_id) {
				var params = {
					"form_id": form_id, 
					"msg_id": msg_id
				};
				IKLAD_2423.iframe.contentWindow.postMessage('notify_form_completed:' + "\n" + JSON.stringify(params), '*');
			}
		};
		
		//status JS
		
var IKLAD_STATUS_2423 = {
	TEMPLATE_HTML_CODE: '',	// will hold HTML code for this hint template

	online_img_url: 'http://chat.iklad-chat.biz/images/3000/shama2.png',
	offline_img_url: 'http://chat.iklad-chat.biz/images/3000/shama2.png',
	custom_online_img_url: 'http://',
	custom_offline_img_url: 'http://',
	
	hide_on_offline: 1,
	hide_on_chat_open: 1,
	
	animated_appearance: 1,
	animation_effect: 'fade_in',
	animation_effect_duration: 400,
	
	animation_effect_step: 0,
	animation_effect_total_steps: 50,
	
	
	curr_op_status: '',
	window_open: false,
	
	first_time_appearance: true,
	i_am_visible: '', //not set
				
	init: function (params) {
		//params.operator_status = 'online' || 'ofline' || ''
		
		if (IKLAD_STATUS_2423.online_img_url == '-1') IKLAD_STATUS_2423.online_img_url = IKLAD_STATUS_2423.custom_online_img_url;
		if (IKLAD_STATUS_2423.offline_img_url == '-1') IKLAD_STATUS_2423.offline_img_url = IKLAD_STATUS_2423.custom_offline_img_url;
		
		IKLAD_STATUS_2423.online_img_url = IKLAD_STATUS_2423.online_img_url.replace(/^https?\:/i, '');
		IKLAD_STATUS_2423.offline_img_url = IKLAD_STATUS_2423.offline_img_url.replace(/^https?\:/i, '');
		
		var s = document.createElement('div');
		s.innerHTML = IKLAD_STATUS_2423.TEMPLATE_HTML_CODE;
		IKLAD_2423.CODE_CONTAINER.appendChild(s);
		
		IKLAD_STATUS_2423.operator_status_changed(params);
	},
	
	operator_status_changed: function (params) {
		//params.operator_status: 'online' || 'offline' || 'connecting' || 'error' || '' = unknown
		
		//ignore all statuses except 'online' and 'offline'
		if (! ((params.operator_status == 'online') || (params.operator_status == 'offline'))) return;
		
		if (IKLAD_STATUS_2423.curr_op_status == params.operator_status) return;
		
		IKLAD_STATUS_2423.curr_op_status = params.operator_status;
		IKLAD_STATUS_2423.__INT_change_image();
		IKLAD_STATUS_2423.__INT_show_OR_hide_status();
	},
	
	window_state_changed: function (params) {
		//params.state: 'closed' || 'open' | 'popup'
		IKLAD_STATUS_2423.window_open = params.state == 'open';
		IKLAD_STATUS_2423.__INT_show_OR_hide_status();
	},
	
	
	//this function is used to retrieve coordinates (POINT with margins) where 'hint box' will be displayed
	// should return object:
	// {
	//  top:					]
	//  left:					] [top || bottom] && [left || right] SHOULD BE DEFINED.
	//  right:				] should end with 'px' or '%'.  Ex: '10px',  '50%'
	//  bottom:				]
	//  position: 'absoulte' || 'fixed'
	//
	//	optional:
	//	margin_top, margin_left, margin_right, margin_bottom   - ONLY IN PIXELS!
	// }
	
	get_hint_box_coords: function () {
		var hint_over_object_container = document.getElementById('IKLAD_STATUS_CONTAINER_2423');
		var hint_over_object = document.getElementById('IKLAD_STATUS_IMG_2423');
		
		var res = {};
		
		if (hint_over_object_container.style.position == 'fixed') {
			res.position = 'fixed';
			var browser_size = IKLAD_TEMPLATE_2423.__get_browser_size();
			
			if (hint_over_object_container.style.right) {
				if (hint_over_object_container.style.right.match(/px/)) {
					res.right = parseInt(hint_over_object_container.style.right) + parseInt(hint_over_object_container.offsetWidth/2) + 'px';
				} else {
					res.right = hint_over_object_container.style.right;
					res.margin_right = parseInt(hint_over_object_container.offsetWidth/2);
				};
			};
	
			if (hint_over_object_container.style.left) {
				if (hint_over_object_container.style.left.match(/px/)) {
					res.left = parseInt(hint_over_object_container.style.left) + parseInt(hint_over_object_container.offsetWidth/2) + 'px';
				} else {
					res.left = hint_over_object_container.style.left;
					res.margin_left = parseInt(hint_over_object_container.offsetWidth/2);
				};
			};
	
			if (hint_over_object_container.style.top) {
				if (hint_over_object_container.style.top.match(/px/)) {
					res.top = parseInt(hint_over_object_container.style.top) + parseInt(hint_over_object_container.offsetHeight/2) + 'px';
				} else {
					res.top = hint_over_object_container.style.top;
					res.margin_top = parseInt(hint_over_object_container.offsetHeight/2);
				};
			};
			
			if (hint_over_object_container.style.bottom) {
				if (hint_over_object_container.style.bottom.match(/px/)) {
					res.bottom = parseInt(hint_over_object_container.style.bottom) + parseInt(hint_over_object_container.offsetHeight/2) + 'px';
				} else {
					res.bottom = hint_over_object_container.style.bottom;
					res.margin_bottom = parseInt(hint_over_object_container.offsetHeight/2);
				};
			};
			
				
			if (hint_over_object_container.style.marginRight) {
				if (res.margin_right) {
					res.margin_right = parseInt(res.margin_right) + parseInt(hint_over_object_container.style.marginRight) + 'px';
				} else {
					res.margin_right = hint_over_object_container.style.marginRight;
				};
			};
	
			if (hint_over_object_container.style.marginLeft) {
				if (res.margin_left) {
					res.margin_left = parseInt(res.margin_left) + parseInt(hint_over_object_container.style.marginLeft) + 'px';
				} else {
					res.margin_left = hint_over_object_container.style.marginLeft;
				};
			};
			
			if (hint_over_object_container.style.marginTop) {
				if (res.margin_top) {
					res.margin_top = parseInt(res.margin_top) + parseInt(hint_over_object_container.style.marginTop) + 'px';
				} else {
					res.margin_top = hint_over_object_container.style.marginTop;
				};
			};
			
			if (hint_over_object_container.style.marginBottom) {
				if (res.margin_bottom) {
					res.margin_bottom = parseInt(res.margin_bottom) + parseInt(hint_over_object_container.style.marginBottom) + 'px';
				} else {
					res.margin_bottom = hint_over_object_container.style.marginBottom;
				};
			};
			
		} else {
			res.position = 'absolute';
			var Pos = IKLAD_FUNC.find_object_position(hint_over_object);
			res.left = Pos[0] + parseInt(hint_over_object.offsetWidth/2) + 'px';
			res.top = Pos[1] + parseInt(hint_over_object.offsetHeight/2) + 'px';
		};
		
		return res;
	},
	
	
	
	
	
	
	
	
	/////////////////////////// INTERNAL FUNCITONS /////////////////////////////////
	
	__INT_change_image: function () {
		var img_data;
		
		if (IKLAD_STATUS_2423.curr_op_status == 'online') {
			img_data = '<a href="javascript: IKLAD_TEMPLATE_2423.open_chat_window()"><img id="IKLAD_STATUS_IMG_2423" src="' + IKLAD_STATUS_2423.online_img_url + '" border=0></a>';
		} else {
			img_data = '<a href="javascript: IKLAD_TEMPLATE_2423.open_chat_window()"><img id="IKLAD_STATUS_IMG_2423" src="' + IKLAD_STATUS_2423.offline_img_url + '" border=0></a>';
		};
		
		var container = document.getElementById('IKLAD_STATUS_CONTAINER_2423');
		container.style.display = 'none';
		if (container) { container.innerHTML = img_data; };
	},
	
	
	
	__INT_show_OR_hide_status: function () {
		var img_visible = true;
		
		if (IKLAD_STATUS_2423.curr_op_status == 'online') {
			
			if (IKLAD_STATUS_2423.window_open) {
				img_visible = ! IKLAD_STATUS_2423.hide_on_chat_open;
			} else {
				img_visible = true;
			};
			
		} else {		//op OFFLINE
			
			if (IKLAD_STATUS_2423.window_open) {
				img_visible = ! IKLAD_STATUS_2423.hide_on_chat_open;
			} else {
				img_visible = ! IKLAD_STATUS_2423.hide_on_offline;
			};
		};
		
		
		if (img_visible) {
			
			if (IKLAD_STATUS_2423.i_am_visible != '1') {
				IKLAD_STATUS_2423.i_am_visible = '1';
				
				
				
				if (IKLAD_STATUS_2423.first_time_appearance && (IKLAD_STATUS_2423.animated_appearance == 1)) {
					IKLAD_STATUS_2423.first_time_appearance = false;
					IKLAD_STATUS_2423.__INT_appear_animated();
				} else {
					document.getElementById('IKLAD_STATUS_CONTAINER_2423').style.display = '';
				};
				
				
				
			};
			
		} else {
			
			if (IKLAD_STATUS_2423.i_am_visible != '0') {
				IKLAD_STATUS_2423.i_am_visible = '0';
				document.getElementById('IKLAD_STATUS_CONTAINER_2423').style.display = 'none';
			};
			
		};

		
	},
	
	

	__INT_appear_animated: function () {
		
		if (IKLAD_STATUS_2423.animation_effect == 'fade_in') return IKLAD_STATUS_2423.__INT_appear_animated_FADE_IN();
		
		
		
		//unknown animation effect
		document.getElementById('IKLAD_STATUS_CONTAINER_2423').style.display = '';
	},
	
	
	__INT_appear_animated_FADE_IN: function () {
		document.getElementById('IKLAD_STATUS_CONTAINER_2423').style.opacity = 0;
		document.getElementById('IKLAD_STATUS_CONTAINER_2423').style.display = '';
		
		if (typeof jQuery == 'undefined') {
			IKLAD_STATUS_2423.animation_effect_step = 0;
			IKLAD_STATUS_2423.__INT_appear_animated_FADE_IN_STEP();
		} else {
			jQuery('#IKLAD_STATUS_CONTAINER_2423').css({'opacity': 0}).animate({'opacity': 1}, {'duration': IKLAD_STATUS_2423.animation_effect_duration});
		};
	},
	
	__INT_appear_animated_FADE_IN_STEP: function () {
		IKLAD_STATUS_2423.animation_effect_step++;
		
		if (IKLAD_STATUS_2423.animation_effect_step >= IKLAD_STATUS_2423.animation_effect_total_steps) {
			document.getElementById('IKLAD_STATUS_CONTAINER_2423').style.opacity = 1;
			return;
		};
		
		var opc = IKLAD_STATUS_2423.animation_effect_step / IKLAD_STATUS_2423.animation_effect_total_steps;
		document.getElementById('IKLAD_STATUS_CONTAINER_2423').style.opacity = opc;
		
		setTimeout(IKLAD_STATUS_2423.__INT_appear_animated_FADE_IN_STEP, Math.round(IKLAD_STATUS_2423.animation_effect_duration / IKLAD_STATUS_2423.animation_effect_total_steps));
	},
	



	
	
	
	
	
	last_element_for_IE: ''
};

		IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += '<!-- define styles here -->';
IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += '<style type="text/css">';
IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += '	#IKLAD_STATUS_CONTAINER_2423 {';
IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += '	}';
IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += '	';
IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += '	#IKLAD_STATUS_IMG_2423 {';
IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += '	    vertical-align: bottom;';
IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += '	}';
IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += '</style>';
IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += '';
IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_STATUS_2423.TEMPLATE_HTML_CODE += '<div style=" position: fixed; right:0; bottom:0;  margin-right:0px; margin-bottom:0px;     z-index:2100000000;" id="IKLAD_STATUS_CONTAINER_2423" class="IKLAD_STATUS_CONTAINER_2423"></div>';


		//hint template
		
var IKLAD_HINT_2423 = {
	TEMPLATE_HTML_CODE: '',	// will hold HTML code for this hint template
	
	hint_container: '',
	hint_text_container: '',
	
	init: function () {		
		var d = document.createElement('div');
		d.setAttribute("style", "position: absolute;");
		d.style.position = 'absolute';
		d.style.top = '0px';
		d.style.left = '0px';
		d.innerHTML = IKLAD_HINT_2423.TEMPLATE_HTML_CODE;
		document.body.appendChild(d);
		
		IKLAD_HINT_2423.hint_container = document.getElementById('IKLAD_HINT_CONTAINER_2423');
		IKLAD_HINT_2423.hint_text_container = document.getElementById('IKLAD_HINT_TEXT_CONTAINER_2423');
	},
	
	window_state_changed: function (params) {
		//params.state: 'closed' || 'open' | 'popup'
		
		//process only 'open' state
		if (params.state == 'open') {
			IKLAD_HINT_2423.hide();
		};
		
	},
	
	show: function (msg_text, show_point) {
		if (! IKLAD_HINT_2423.hint_container) return;
		if (! IKLAD_HINT_2423.hint_text_container) return;
		
		IKLAD_HINT_2423.hint_text_container.innerHTML = msg_text;
		
		var hint_container = IKLAD_HINT_2423.hint_container;
		 
		var positionClass_1;
		var positionClass_2;
		var hint_container_Width = 248;
		var hint_container_Height = 207;
		
		hint_container.style.position = show_point.position;
		
		
		hint_container.style.display = 'none';	
		hint_container.style.top = null;
		hint_container.style.bottom = null;
		hint_container.style.right = null;
		hint_container.style.left = null;
		
		hint_container.style.marginTop = null;
		hint_container.style.marginBottom = null;
		hint_container.style.marginLeft = null;
		hint_container.style.marginRight = null;
		
		
		if (show_point.position == 'absoulte') {

			
			/*
			var trueBody = (document.compatMode && document.compatMode!="BackCompat")? document.documentElement : document.body;
			
      if ( parseInt(trueBody.clientWidth + trueBody.scrollLeft) < parseInt(hint_container_Width + Pos[0])) {
        positionClass = "IKLAD_Right";
        Pos[0] -= hint_container_Width;
      } else {
	      positionClass = "IKLAD_Left";
      };
      
      if ( parseInt(trueBody.clientHeight + trueBody.scrollTop) < parseInt(hint_container_Height + Pos[1])) {
        positionClass += "Bottom";
        Pos[1] -= hint_container_Height;
      } else {
        positionClass += "Top";
      };
      
      */
		};
		
	
		var browser_size = IKLAD_HINT_2423.__get_browser_size();
		
		var corner_left_right = '';
		var corner_top_bottom = '';
		
		if (show_point.left) {
			hint_container.style.left = show_point.left;
			
			if (show_point.left.match(/px/)) {
				corner_left_right = (parseInt(show_point.left) <= (browser_size.width/2)) ? 'left' : 'right';
			} else {
				corner_left_right = (parseInt(show_point.left) <= 50) ? 'left' : 'right';
			};
		};
		
		if (show_point.right) {
			hint_container.style.right = show_point.right;
			
			if (show_point.right.match(/px/)) {
				corner_left_right = (parseInt(show_point.right) <= (browser_size.width/2)) ? 'right' : 'left';
			} else {
				corner_left_right = (parseInt(show_point.right) <= 50) ? 'right' : 'left';
			};
		};
		
		if (show_point.top) {
			hint_container.style.top = show_point.top;
			
			if (show_point.top.match(/px/)) {
				corner_top_bottom = (parseInt(show_point.top) <= (browser_size.height/2)) ? 'top' : 'bottom';
			} else {
				corner_top_bottom = (parseInt(show_point.top) <= 50) ? 'top' : 'bottom';
			};
		};
		
		if (show_point.bottom) {
			hint_container.style.bottom = show_point.bottom;
			
			if (show_point.bottom.match(/px/)) {
				corner_top_bottom = (parseInt(show_point.bottom) <= (browser_size.height/2)) ? 'bottom' : 'top';
			} else {
				corner_top_bottom = (parseInt(show_point.bottom) <= 50) ? 'bottom' : 'top';
			};
		};
		
		
		
		show_point.margin_left = parseInt(typeof show_point.margin_left != 'undefined' ? show_point.margin_left : 0);
		show_point.margin_right = parseInt(typeof show_point.margin_right != 'undefined' ? show_point.margin_right : 0);
		show_point.margin_top = parseInt(typeof show_point.margin_top != 'undefined' ? show_point.margin_top : 0);
		show_point.margin_bottom = parseInt(typeof show_point.margin_bottom != 'undefined' ? show_point.margin_bottom : 0);
		
		
		if (corner_left_right == 'left') {
			positionClass_1 = "IKLAD_Left";
			
			if (show_point.left) {
				hint_container.style.marginLeft = show_point.margin_left + 'px';
			} else {
				hint_container.style.marginRight = -(hint_container_Width - show_point.margin_right) + 'px';
			};
			
		} else {
			positionClass_1 = "IKLAD_Right";
			
			if (show_point.left) {
				hint_container.style.marginLeft = -(hint_container_Width - show_point.margin_left) + 'px';
			} else {
				hint_container.style.marginRight = show_point.margin_right + 'px';
			};

		};
		
		
		if (corner_top_bottom == 'top') {
			positionClass_2 = "Top";
			
			if (show_point.top) {
				hint_container.style.marginTop = show_point.margin_top + 'px';
			} else {
				hint_container.style.marginBottom = -(hint_container_Height - show_point.margin_bottom) + 'px';
			};
			
		} else {
			positionClass_2 = "Bottom";
			
			if (show_point.top) {
				hint_container.style.marginTop = -(hint_container_Height - show_point.margin_top) + 'px';
			} else {
				hint_container.style.marginBottom = show_point.margin_bottom + 'px';
			};

		};
						
		
		hint_container.className = positionClass_1 + positionClass_2 + '_2423';
		hint_container.style.display = '';
	},
	
	
	hide: function (event) {
		if (! IKLAD_HINT_2423.hint_container) return;
		IKLAD_HINT_2423.hint_container.style.display = 'none';
		
		try { event.stopPropagation(); event.cancelBubble = true; } catch (r) {};
	},
	
	
	
	
	
	
	
	
	/////////////// INTERNAL FUNC ///////////////
	
	__my_open_chat: function () {
		IKLAD_HINT_2423.hide(); 
		IKLAD_TEMPLATE_2423.open_chat_window();
	},
	
	__get_browser_size: function () {
		var e = window, a = 'inner';
		if ( !( 'innerWidth' in window ) ) {
			a = 'client';
			e = document.documentElement || document.body;
		};
		return { width : e[ a+'Width' ] , height : e[ a+'Height' ] }
	}
};
		IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '<style type="text/css">';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	div.IKLAD_RightBottom_2423 {';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		background: url(https://chat.iklad-chat.biz/img/hint_template/0/rb.png) no-repeat;';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	}';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	div.IKLAD_RightTop_2423 {';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		background: url(https://chat.iklad-chat.biz/img/hint_template/0/rt.png) no-repeat;';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	}';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	div.IKLAD_LeftBottom_2423 {';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		background: url(https://chat.iklad-chat.biz/img/hint_template/0/lb.png) no-repeat;';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	}';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	div.IKLAD_LeftTop_2423 {';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		background: url(https://chat.iklad-chat.biz/img/hint_template/0/lt.png) no-repeat;';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	}';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	div.IKLAD_HINT_BTN_2423 {';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		border: 1px solid lightgray; ';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	  color: gray; ';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	  cursor: pointer;';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	  padding: 3px 5px;';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	  float: left;';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	  font-size: 12px;';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	}';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	#IKLAD_HINT_CONTAINER_2423 {';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		border: 0; ';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		margin: 0; ';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		padding: 0; ';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		text-align: left;';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		z-index: 2100000100;';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	}';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '</style>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '<div id="IKLAD_HINT_CONTAINER_2423" style="display: none; position: absolute;">';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	<div style="width: 248px; height: 207px; margin: 0; padding: 0; text-align: left;">';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		<div onclick="IKLAD_HINT_2423.__my_open_chat();" style="cursor: pointer; position: relative; width: 200px; height: 148px; overflow: auto; text-align: left; margin: 0; padding: 5px; top: 23px; left: 18px;">';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '		  ';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '			<div style="float: right; border: 1px solid lightgray; padding: 0px 3px; color: lightgray; cursor: pointer; margin-left: 1px; margin-bottom: 5px; display: none;" id="IKLAD_HINT_CLOSE_BTN_X_2423" onclick="IKLAD_HINT_2423.hide(event);" title="&#67;&#108;&#111;&#115;&#101;">X</div>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '			';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '			<span id="IKLAD_HINT_TEXT_CONTAINER_2423" style=" color: black; font-size: 14px;"></span>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '			';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '			<div style="position: absolute; bottom:0; width: 190px;">';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '       <table style="margin: 0 auto;">';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '        <tr>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '          <td>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '      			<div style="" class="IKLAD_HINT_BTN_2423" id="IKLAD_HINT_START_CHAT_BTN_2423" onclick="IKLAD_HINT_2423.__my_open_chat();">&#83;&#116;&#97;&#114;&#116;&#32;&#99;&#104;&#97;&#116;</div>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '					  <div style="margin-left: 5px; " class="IKLAD_HINT_BTN_2423" id="IKLAD_HINT_CLOSE_BTN_2423" onclick="IKLAD_HINT_2423.hide(event);">&#67;&#108;&#111;&#115;&#101;</div>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '          </td>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '          ';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '        </tr>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '       </table>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '			</div>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '	</div>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";IKLAD_HINT_2423.TEMPLATE_HTML_CODE += '</div>';
IKLAD_HINT_2423.TEMPLATE_HTML_CODE += "\n";
		
		//form JS
		var IKLAD_NEED_JQUERY_2423 = 0;
var IKLAD_NEED_JQUERY_UI_2423 = 0;


<!-- we need this for form show/hide animation -->
	IKLAD_NEED_JQUERY_2423 = 1;






if ((typeof jQuery == 'undefined') && (IKLAD_NEED_JQUERY_2423 == 1)) {
	var s = document.createElement('script');
	s.type = 'text/javascript';
	s.charset = 'utf-8';
	s.src = 'https://ajax.googleapis.com/ajax/libs/jquery/1.11.2/jquery.min.js';

	if (IKLAD_NEED_JQUERY_UI_2423 == 1) {
		s.onload = function () {
			var s_ui = document.createElement('script');
			s_ui.type = 'text/javascript';
			s_ui.charset = 'utf-8';
			s_ui.src = 'https://code.jquery.com/ui/1.11.2/jquery-ui.min.js';
			document.body.appendChild(s_ui);
		};
	};

	document.body.appendChild(s);
};




var IKLAD_TEMPLATE_2423 = {
	//mandatory functions
	curr_operator_status: '',
	__offline_actions: [],
	
	init: function (params) {
		//params.operator_status = 'online' || 'offline'
		
		//params.opinions :  array of objects:
		// {
		//  id: id of opinion
		//  text: text to diaply
		//	want_comment: 1|0  whether or not ask user for comments when choosing this variant
		// }, ... {}, {} ...

		
		//init 'messages' iframe
		var fr = window.frames.IKLAD_MSGS_IFR_2423;
		var d = fr.document;
		d.open();
		d.write('<html><head><META HTTP-EQUIV="Pragma" CONTENT="No-Cache"><META HTTP-EQUIV="CACHE-CONTROL" CONTENT="NO-CACHE"><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><base target="_Blank"></head>');
		d.write('<body leftmargin="0" topmargin="0" rightmargin="0" bottommargin="0" marginwidth="0" marginheight="0">');
		
		d.write('	\
			<style type="text/css"> \
			::-webkit-scrollbar {	\
				width: 12px;	\
			}	\
			::-webkit-scrollbar-track {	\
				background-color: #ebf1f8;	\
			}	\
			::-webkit-scrollbar-thumb {	\
				background-color: #abb7c8;	\
			}	\
      body { \
        background-color: #ecf3f8; \
				font-family: Verdana; \
      } \
			table { \
				padding: 0; \
				margin: 0; \
				border-collapse: collapse; \
				border-spacing: 0; \
			} \
			div.msg_line { \
				margin: 4px 0 7px 0; \
				width: 100%; \
				box-sizing: border-box; \
			} \
			div.msg_line_form { \
				padding: 0 40px 0 8px; \
			}\
			td.msg_from_operator_left { \
				width: 10px; \
				background-image: url(\'data:image/png;base64,\'); \
				background-repeat: no-repeat; \
				background-position: 1px 50%; \
			} \
			div.msg_from_operator { \
				border-radius: 6px;	\
				background: #c0ec86;	\
				font-size: 12px; \
				padding: 7px 7px 7px 7px; \
        box-shadow: 1px 1px 5px rgba(0, 0, 0, 0.2); \
        word-wrap: break-word; \
        word-break: break-all; \
			}	\
			td.msg_from_operator_time, td.msg_from_client_time { \
				width: 40px; \
				text-align: center; \
				color: #c7c7c7; \
				font-size: 10px; \
        font-family: Helvetica; \
			} \
			td.msg_from_client_right { \
				width: 10px; \
				background-image: url("data:image/png;base64,"); \
				background-repeat: no-repeat; \
				background-position: -1px 50%; \
			} \
			div.msg_from_client { \
				border-radius: 6px;	\
				background: #ffffff;	\
				font-size: 12px; \
				padding: 7px 7px 7px 7px; \
        box-shadow: -1px 1px 5px rgba(0, 0, 0, 0.2); \
        word-wrap: break-word; \
        word-break: break-all; \
			}	\
			#MSGS_CONTAINER { \
			} \
			</style>	\
		');
		
		d.write('<div id="BEFORE_MSGS_CONTAINER" style="display: block; background: linear-gradient(to bottom, #d8e8f2, #ecf3f8); height: 3px;"></div>');
		d.write('<div id="MSGS_CONTAINER"></div>');
		d.write('<div id="OPERATOR_TYPING_CONTAINER" style="display: none; opacity: 0.5; padding: 0 0 7px 3px;">	\
			<table width=100% cellpadding=0 cellspacing=0><tr> \
			<td class="msg_from_operator_left"></td> \
			<td class="msg_from_operator"><div class="msg_from_operator" id="OPERATOR_TYPING_MSG" style="font-size: 11px;"></div></td> \
			<td class="msg_from_operator_time">&nbsp;</td> \
			</tr></table></div>');
		d.write('</bo' + 'dy></ht' + 'ml>');
		d.close();
		
		IKLAD_TEMPLATE_2423.msgs_ifr = fr;
		IKLAD_TEMPLATE_2423.msgs_container = d.getElementById('MSGS_CONTAINER');
		
		//here NOT d. , but DOCUMENT 
		IKLAD_TEMPLATE_2423.top_form_container_tr = document.getElementById('TOP_FORM_CONTAINER_TR');
		IKLAD_TEMPLATE_2423.top_form_container = document.getElementById('TOP_FORM_CONTAINER');
		IKLAD_TEMPLATE_2423.bottom_form_container_tr = document.getElementById('BOTTOM_FORM_CONTAINER_TR');
		IKLAD_TEMPLATE_2423.bottom_form_container = document.getElementById('BOTTOM_FORM_CONTAINER');
		
		IKLAD_TEMPLATE_2423.operator_typing_container = d.getElementById('OPERATOR_TYPING_CONTAINER');
		IKLAD_TEMPLATE_2423.operator_typing_msg_container = d.getElementById('OPERATOR_TYPING_MSG');
		
		IKLAD_TEMPLATE_2423.__iframe_import_CSS();
		
		//init 'leave opinion dialog'
		var opinions_html_code = '<table cellpadding=0 cellspacing=0 style="margin: 0; padding: 0;">';
		opinions_html_code += '<tr id="IKLAD_OPINION_HEADER_2423"><td>Оставить отзыв</td><td width=10 style="padding: 0 1px 0 5px !important;" align=center><a style="text-decoration: none;" href="#" onclick="IKLAD_TEMPLATE_2423.local_hide_opinion_dialog(); return false;" title="закрыть">x</a></td></tr>';

		for (var i=0; i < params.opinions.length; i++) {
			opinions_html_code += '<tr>';
			opinions_html_code += '<td colspan=2 class="IKLAD_OPINION_LIST_2423" onclick="javascript:IKLAD_TEMPLATE_2423.local_opinion_set(' + params.opinions[i].id + ',' + params.opinions[i].want_comment + ')">' + params.opinions[i].text + '</td>';
			opinions_html_code += '</tr>';
		};

		opinions_html_code += '</table>';

		var dv = document.createElement('div');
		dv.setAttribute('id', 'IKLAD_OPINION_BOX_2423');
		dv.setAttribute('style', 'z-index: 100000000000;');
		dv.style.zIndex = 100000000000;
		dv.innerHTML = opinions_html_code;
		dv.style.display = 'none';
		document.body.appendChild(dv);
		
		IKLAD_TEMPLATE_2423.original_form_height = document.getElementById('IKLAD_SET_HEIGHT_TD_2423').style.height;
	},
	

	//	
	__iframe_import_CSS: function () {
		var fr = IKLAD_TEMPLATE_2423.msgs_ifr;
		var css = document.getElementById('IKLAD_CSS_2423');
		if (! css) return;
		
    var st = fr.document.createElement("style");
    st.type = "text/css";
    try {
     st.innerHTML = css.innerText;
    } catch (ex) {
     st.styleSheet.cssText = css.innerText;  // IE8 and earlier
    };
    
    fr.document.getElementsByTagName("head")[0].appendChild(st);
	},
	
	
	operator_status_changed: function (params) {
		//params.operator_status = 'online' || 'offline' || 'connecting' || 'error' | 'no_empty_accounts'
		
		//--- params.operator_status == 'online' || 'offline'
		//>>>no additional params		
		
		//--- params.operator_status == 'offline'
		//>>> params.actions = [			// list of actions to do
		//>>>   {
		//>>>   	action: 'form' | 'message'
		//>>>   	param: 'form_ID' | 'message_text'
		//>>>   }
		//>>> ]
		
		//--- params.operator_status == 'connecting' || 'no_empty_accounts'
		//>>> no additional params
		
		//--- params.operator_status == 'error'
		//>>> params.error_message => text error
		
		var img_el = document.getElementById('IKLAD_OP_STATUS_IMG_2423');
		var text_el = document.getElementById('IKLAD_OP_STATUS_TEXT_2423');
		
		var __online_data = 'data:image/png;base64,';
		var __offline_data = 'data:image/png;base64,';
		var __loading = 'data:image/gif;base64,';
		
		if (params.operator_status == 'connecting') {
			img_el.src = __loading;
			text_el.innerHTML = 'подключение';
		};
	

		if (params.operator_status == 'online') {
			img_el.src = __online_data;
			text_el.innerHTML = 'онлайн';
		};
		
		if (params.operator_status == 'offline') {
			img_el.src = __offline_data;
			text_el.innerHTML = 'оффлайн';
		};
		
		if (params.operator_status == 'no_empty_accounts') {
			img_el.src = __offline_data;
			text_el.innerHTML = 'нет свободных каналов';
		};
		
		if (params.operator_status == 'error') {
			img_el.src = __offline_data;
			text_el.innerHTML = 'ошибка соединения с оператором';
		};
		
		
		//////////
		var online_offline_op_status;
		
		if (! ((params.operator_status == 'online') || (params.operator_status == 'offline')) ) {
			return; // NO FUTHER procrssing of statuses. only online and offline
		};
		
		if (IKLAD_TEMPLATE_2423.curr_operator_status == params.operator_status) return;
		IKLAD_TEMPLATE_2423.curr_operator_status = params.operator_status;
		
		if (params.operator_status == 'online') {
			IKLAD_TEMPLATE_2423.__show_layer('online');
		}; 

		if (params.operator_status == 'offline') {
			IKLAD_TEMPLATE_2423.__offline_actions = params.actions;
			IKLAD_TEMPLATE_2423.__show_layer('offline');
		}; 
	},
	
	
	__show_layer: function (layer) {
		
		if (layer == 'default') {
			if (IKLAD_TEMPLATE_2423.curr_operator_status == 'online') {
				layer = 'online';
			};
			
			if (IKLAD_TEMPLATE_2423.curr_operator_status == 'offline') {
				layer = 'offline';
			};
		};
		
		//for state 'connecting', etc, .
		if (layer == 'default') {
			layer = 'loading';
		};
				
		
		var loading_layer_visible = false;
		var modal_layer_visible = false;
		var online_layer_visible = false;
		var footer_visible = false;
		var textarea_visible = false;
		
		var reset_from_height = false;
		var set_original_from_height = true;
		
		if (layer == 'online') {
			online_layer_visible = true;
			footer_visible = true;
			textarea_visible = true;
		};
	
		if (layer == 'loading') {
			loading_layer_visible = true;
		};
		
		if (layer == 'modal_form') {
			modal_layer_visible = true;
			reset_from_height = true;
		};
	
	
		if (layer == 'offline') {
			
			for (var i=0; i < IKLAD_TEMPLATE_2423.__offline_actions.length; i++) {
				var this_action = IKLAD_TEMPLATE_2423.__offline_actions[i];
				
				if (this_action.action == 'form') {
					IKLAD_TEMPLATE_2423.local_on_chat_message({
						"type": 'form',
						"text": this_action.param, 
						"to": "client", 
						"dt": "",
						"id": 0,
						"is_info": 1,
						"by_human": 0
					});
					continue;
				};
				
				
				if (this_action.action == 'message') {
					online_layer_visible = true;
					
					IKLAD_TEMPLATE_2423.local_on_chat_message({
						"type": 'msg',
						"text": this_action.param, 
						"to": "client", 
						"dt": "", 
						"id": 0, 
						"is_info": 1,
						"by_human": 0
					});
					
					continue;
				};
			};
			
		};
	
	
		document.getElementById('LOADING_LAYER_2423').style.display = loading_layer_visible ? '' : 'none';
		document.getElementById('MODAL_FORM_LAYER_2423').style.display = modal_layer_visible ? '' : 'none';
		document.getElementById('ONLINE_LAYER_2423').style.display = online_layer_visible ? '' : 'none';
		
		document.getElementById('IKLAD_TEXTAREA_ROW_CONTAINER_2423').style.display = textarea_visible ? '' : 'none';
		//document.getElementById('IKLAD_FOOTER_2423').style.display = footer_visible ? '' : 'none';
		
		if (reset_from_height) {
			document.getElementById('IKLAD_SET_HEIGHT_TD_2423').style.height = null;
		};
		
		if (set_original_from_height && (! reset_from_height)) {
			document.getElementById('IKLAD_SET_HEIGHT_TD_2423').style.height = IKLAD_TEMPLATE_2423.original_form_height;
		};
	},
	
	
	operator_data_changed: function (params) {
		//params.name - name of currently assigned operator
		//params.photo - abs path to image
		
		if (typeof params.photo != 'undefined') {
			document.getElementById('IKLAD_OPERATOR_PHOTO_2423').src = params.photo;
		};
	},
	
	
	operator_activity: function (params) {
		//params.type == 'operator_typing'			- operator keyboard activity
		//params.status = 'composing' || 'paused' || ''				('' = clear status)
		
		if (params.type == 'operator_typing') {
			
			if (params.status == '') {
				IKLAD_TEMPLATE_2423.operator_typing_container.style.display = 'none';
				IKLAD_TEMPLATE_2423.operator_typing_msg_container.innerHTML = '';
			};
			
			if (params.status == 'paused') {
				IKLAD_TEMPLATE_2423.operator_typing_container.style.display = '';
				IKLAD_TEMPLATE_2423.operator_typing_msg_container.innerHTML = 'оператор приостановил печать ...';
			};
			
			if (params.status == 'composing') {
				IKLAD_TEMPLATE_2423.operator_typing_container.style.display = '';
				IKLAD_TEMPLATE_2423.operator_typing_msg_container.innerHTML = '<img src="//chat.iklad-chat.biz/img/templates/design3/composing.gif" style="padding-right:4px;">оператор печатает ...';
			};
			
			IKLAD_TEMPLATE_2423.scrol_msg_list_to_bottom();
		};
		
	},
	
	open_chat_window: function (params) {
		IKLAD_TEMPLATE_2423.tmpl_open_window();
		
		//notify 'core' that chat_window is open now 
		IKLAD_2423.notify_window_state_changed({state: 'open'});
		IKLAD_TEMPLATE_2423.chat_window_open = true;
		
		IKLAD_TEMPLATE_2423.scrol_msg_list_to_bottom();
	},
	
	close_chat_window: function (params) {
		IKLAD_TEMPLATE_2423.tmpl_close_window();
		
		//notify 'core' that chat_window is closed now 
		IKLAD_2423.notify_window_state_changed({state: 'closed'});
		IKLAD_TEMPLATE_2423.chat_window_open = false;
	},
	
	
	full_chat_log_data: function (params) {
		//params.msgs: array of 'msg_obj': (see operator_message function for description)
		
		IKLAD_TEMPLATE_2423.msgs_container.innerHTML = '';
		IKLAD_TEMPLATE_2423.msgs_count = 0;
		
		for (var i=0; i < params.msgs.length; i++) {
			IKLAD_TEMPLATE_2423.local_on_chat_message(params.msgs[i], false);
		};
	},
	
	on_chat_message: function (msg_obj) {
		// msg_obj: {
		//	type: msg || form					(text = form_id for [type=form])
		//	completed: 0 | 1   (ONLY FOR FORM)
		//  text: text of message	(already processed for smiles/html chars, so it's ready to 'display') OR form_id
		//  dt: datetime of message in format 'YYYY-MM-DD HH:MM:SS'
		//	to: 'client' || 'operator'
		//  id: internal id of msg
		//  is_info: 1|0  (1 = server info msg)
		//  operator_name: name of opertor who wrote this msg
		//  show_operator_name: 1|0 display operator name or not (set to 0 for welcome msg)
		//  by_human: 0|1   (1= wrote by human, 0=generated automatically by autioinitiate rule or in other way)
		// }
		
		IKLAD_TEMPLATE_2423.local_on_chat_message(msg_obj, true);
	},
	
	//
	__QF_cached_form_data: [],
	__QF_blink_field_error_count: 10,
		
	
	//form data callback by call to 'get_form_data'
	form_data: function (obj) {
		//obj.msg_id - id of message
		
		//obj.form_data::
		//form_data.ID - int
		
		//form_data.options = {}
		//form_data.options.display	 "modal", "top", "bottom", "inchat"
		
		
		//form_data.fields = []
		//form_data.fields ITEM: {
		//	ID: ...
		//	name: ...
		//	var_name: ...
		//	type: 'int', 'string', 'select', 'select-row', 'select-column', button   (+ SPECIAL:  'header' and 'title' )
		//	options: {
		//		required: 0|1
		//		lines: INT			(1 = input text,  >=2  - textarea)
		//		min_length: INT  (min input length)
		//		type: 		'submit', 'close_chat', 'close_form'  (for BUTTON)
		//	}
		//	default_value: 	'default value is here'
		//	values: [
		//		'value|display_value'    or only 'display_value'
		//		....
		//	]
		//}
		
		var CLASS_FOR_REPLACE = 'REPLACE_CLASS_2423';
		
		var js_call_prefix = (obj.form_data.options.display == 'inchat') ? 'parent.' : '';
		var rows = [];
		var cols = [];
		
		for (var i=0; i < obj.form_data.fields.length; i++) {
			var field = obj.form_data.fields[i];
			
			if (field.type == 'header') {
				rows.push(['<tr class="FORM_HEADER_2423 ' + CLASS_FOR_REPLACE + '">', '</tr>']);
				cols.push(['<td>', '<h1>' + field.name + '</h1>', '</td>']);
				continue;
			};
			
			if (field.type == 'title') {
				rows.push(['<tr class="FORM_TITLE_2423 ' + CLASS_FOR_REPLACE + '">', '</tr>']);
				cols.push(['<td>', '<h3>' + field.name + '</h3>', '</td>']);
				continue;
			};
			
			if (field.type == 'int') {
				if (field.name.length > 0) {
					rows.push(['<tr class="FORM_FIELD_TITLE_2423">', '</tr>']);
					cols.push([
					'<td>', 
						field.name,
					'</td>',
					]);
				};
				
				rows.push(['<tr class="FORM_FIELD_2423 FORM_FIELD_INT_2423 ' + CLASS_FOR_REPLACE + '">', '</tr>']);
				cols.push([
				'<td>', 
				'<input id="FORM_INPUT_2423_' + obj.form_data.ID + '_' + field.ID + '" type="text" name="' + field.var_name + '" value="' + field.value + '" orig_value="' + field.value + '" onfocus="' + js_call_prefix + 'IKLAD_TEMPLATE_2423.__form_el_focus(this, \'' + field.value + '\');" onblur="' + js_call_prefix + 'IKLAD_TEMPLATE_2423.__form_el_blur(this, \'' + field.value + '\');"' + '>', 
				'</td>'
				]);
				
				continue;
			};
			
			
			if (field.type == 'string') {
				if (field.name.length > 0) {
					rows.push(['<tr class="FORM_FIELD_TITLE_2423">', '</tr>']);
					cols.push([
					'<td>', 
						field.name,
					'</td>',
					]);
				};
				
				
				
				rows.push( ['<tr class="FORM_FIELD_2423 FORM_FIELD_STRING_2423 ' + CLASS_FOR_REPLACE + '">', '</tr>'] );
				
				var arr = ['<td>', '', '</td>'];
				
				if (field.options.lines == 1) {
					arr[1] = '<input id="FORM_INPUT_2423_' + obj.form_data.ID + '_' + field.ID + '" type="text" name="' + field.var_name + '" value="' + field.value + '" orig_value="' + field.value + '" onfocus="' + js_call_prefix + 'IKLAD_TEMPLATE_2423.__form_el_focus(this, \'' + field.value + '\');" onblur="' + js_call_prefix + 'IKLAD_TEMPLATE_2423.__form_el_blur(this, \'' + field.value + '\');"' + '>';
				} else {
					arr[1] = '<textarea id="FORM_INPUT_2423_' + obj.form_data.ID + '_' + field.ID + '" rows="' + field.options.lines + '" name="' + field.var_name + '" orig_value="' + field.value + '" onfocus="' + js_call_prefix + 'IKLAD_TEMPLATE_2423.__form_el_focus(this, \'' + field.value + '\');" onblur="' + js_call_prefix + 'IKLAD_TEMPLATE_2423.__form_el_blur(this, \'' + field.value + '\');"' + '>' + field.value + '</textarea>';
				};
				
				cols.push(arr);
				continue;
			};
			
			
			if (field.type == 'select') {
				if (field.name.length > 0) {
					rows.push(['<tr class="FORM_FIELD_TITLE_2423">', '</tr>']);
					cols.push([
					'<td>', 
						field.name,
					'</td>',
					]);
				};
				
				rows.push( ['<tr class="FORM_FIELD_2423 FORM_FIELD_SELECT_2423 ' + CLASS_FOR_REPLACE + '">', '</tr>'] );
				
				
				var select_el_html = '<select id="FORM_INPUT_2423_' + obj.form_data.ID + '_' + field.ID + '" name="' + field.var_name + '">';
				
				for (var k=0; k < field.values.length; k++) {
					var sel = '';
					
					var val, display_value;
					
					if (field.values[k].match(/^(.*?)\|(.*)/)) {
						val = RegExp.$1;
						display_value = RegExp.$2;
					} else {
						val = field.values[k];
						display_value = field.values[k];
					};
					
					if (field.default_value == val) {
						sel = 'selected';
					};
					
					select_el_html += '<option value="' + val + '"' + sel + ' >' + display_value + '</option>';
				};
				
				select_el_html += '</select>';
				
				
				cols.push(['<td>', select_el_html, '</td>']);
				continue;
			};
			
			
			if (field.type == 'select-column') {
				for (var k=0; k < field.values.length; k++) {
					//html += '<tr class="FORM_FIELD_2423 FORM_FIELD_SELECT_COL_2423 ' + CLASS_FOR_REPLACE + '"><td>';
					//html += '<div onclick="IKLAD_TEMPLATE_2423.__FormFieldSelectCol(' + obj.form_data.ID + ', ' + field.ID + ')" id="FORM_FIELD_SELECT_COL_2423_' + obj.form_data.ID + '_' + field.ID + '">' + field.values[k] + '</div>';
					//html += '</td></tr>';
				};
				continue;
			};
			
			
			if (field.type == 'button') {
				var this_btn_html = '<button id="FORM_BUTTON_2423_' + obj.form_data.ID + '_' + field.ID + '" name="' + field.var_name + '" value="' + field.value + '" onclick="' + js_call_prefix + 'IKLAD_TEMPLATE_2423.__QF_button_click(' + obj.form_data.ID + ', ' + obj.msg_id + ', \'' + field.options.type + '\')">' + field.name + '</button>';
				
				if ((field.options.on_prev_line == 1) && (cols.length > 0)) {
					rows[rows.length-1][0] = rows[rows.length-1][0].replace(CLASS_FOR_REPLACE, 'FORM_BUTTON_CONTAINER_2423 ' + CLASS_FOR_REPLACE);
					
					var col_html = field.options.html_container;
					col_html = col_html.replace(field.options.line_1_var, cols[cols.length-1][1]);
					col_html = col_html.replace(field.options.line_2_var, this_btn_html);
					cols[cols.length-1][1] = col_html;
					
				} else {
					rows.push( ['<tr class="FORM_BUTTON_CONTAINER_2423 ' + CLASS_FOR_REPLACE + '">', '</tr>'] );
					cols.push( ['<td colspan=2>', this_btn_html, '</td>'] );
				};

				continue;
			};
			

		};
		
		var html = '<table id="FORM_2423" cellpadding=0 cellspacing=0 style="width: 100%;">';
		for (var i=0; i < rows.length; i++) {
			rows[i][0] = rows[i][0].replace(CLASS_FOR_REPLACE, '');
			
			html += rows[i][0];
			
			for (var k=0; k < cols[i].length; k++) {
				html += cols[i][k];
			};
			
			html += rows[i][1];
		};
		html += '</table>';
		
		
		//save for future use
		IKLAD_TEMPLATE_2423.__QF_cached_form_data.push(obj);
		IKLAD_TEMPLATE_2423.__QF_show_form(obj.form_data.options.display, html, obj.msg_id);
	},
	
	
	
	send_form_data_result: function (result) {
		
		var frm_obj = IKLAD_TEMPLATE_2423.__QF_get_cached_form_data(result.form_id, result.msg_id);
		if (! frm_obj) return;
		
		var d = document;
		
		if (frm_obj.form_data.options.display == 'inchat') {
			d = IKLAD_TEMPLATE_2423.msgs_ifr.document;
		};
		
		
		if (result.result == 1) {
			
			for (var i = 0; i < result.cmd.length; i++) {
				var cmd = result.cmd[i];
				
				if (cmd.action == 'CLOSE_FORM') {
					IKLAD_TEMPLATE_2423.__QF_hide_form(frm_obj.form_data.options.display, result.form_id, result.msg_id);					
					IKLAD_2423.notify_form_completed(result.form_id, result.msg_id);
					continue;
				};
				
				if (cmd.action == 'CLOSE_CHAT') {
					IKLAD_TEMPLATE_2423.close_chat_window();
					IKLAD_2423.notify_form_completed(result.form_id, result.msg_id);
					continue;
				};
				
				if (cmd.action == 'FORM_ERROR') {
					var err_processed = false;
					
					if (cmd.error_type == 'field') {
						var field_id;
						
						var fields = d.querySelectorAll('*[id^="FORM_INPUT_2423_' + result.form_id + '_"]');
						
						for (var i=0; i < fields.length; i++) {
							if (fields[i].name == cmd.field) {
								field_id = fields[i].id;
								break;
							};
						};
						
						if (field_id) {
							err_processed = true;
							IKLAD_TEMPLATE_2423.__QF_blink_field_error(frm_obj.form_data.options.display == 'inchat' ? true : false, field_id, 'FORM_INPUT_ERROR_2423', true);
						};
					};
					
					if (! err_processed) alert(cmd.error_msg);
					
					
					//enable all buttons
					var btns = d.querySelectorAll('*[id^="FORM_BUTTON_2423_' + result.form_id + '_"]');
					
					for (var i=0; i < btns.length; i++) {
						var btn = btns[i];
						btn.className = '';
						btn.disabled = false;
					};
					
					continue;
				};
				
				
			};
			
		} else {
		};
	},
	

	
	__QF_get_cached_form_data: function (form_id, msg_id) {
		
		for (var i=0; i < IKLAD_TEMPLATE_2423.__QF_cached_form_data.length; i++) {
			if (! (IKLAD_TEMPLATE_2423.__QF_cached_form_data[i].msg_id == msg_id)) continue;
			if (! (IKLAD_TEMPLATE_2423.__QF_cached_form_data[i].form_data.ID == form_id)) continue;
			return IKLAD_TEMPLATE_2423.__QF_cached_form_data[i];
		};
		
	},
	
	__QF_button_click: function (form_id, msg_id, action) {
		var frm_obj = IKLAD_TEMPLATE_2423.__QF_get_cached_form_data(form_id, msg_id);
		if (! frm_obj) return;
		
		var d = document;
		
		if (frm_obj.form_data.options.display == 'inchat') {
			d = IKLAD_TEMPLATE_2423.msgs_ifr.document;
		};
		
		
		if ((action == 'submit') || (action == 'cancel')) {
			//disable all buttons
			var btns = d.querySelectorAll('*[id^="FORM_BUTTON_2423_' + form_id + '_"]');
			
			for (var i=0; i < btns.length; i++) {
				var btn = btns[i];
				btn.className = 'FORM_BUTTON_DISABLED_2423';
				btn.disabled = true;
			};
		};
		
		
		if (action == 'submit') {		//validatation of fields is NOT needed
			var data = 'action=submit';
			
			var fields = d.querySelectorAll('*[id^="FORM_INPUT_2423_' + form_id + '_"]');
			
			for (var i=0; i < fields.length; i++) {
				var field = fields[i];
				var orig_value = field.getAttribute('orig_value');
				
				if (orig_value && (orig_value == field.value)) {
					//do not send UNFILLED field
				} else {
					
					if (field.type == 'text') {
						data += '&' + field.name + '=' + escape(field.value);
					};
					
					if (field.type == 'select-one') {
						data += '&' + field.name + '=' + escape(field.value);
						data += '&' + field.name + '__TEXT=' + escape(field.options[field.selectedIndex].text);
					};
				};
			};
			
			IKLAD_2423.send_form_data(form_id, msg_id, data);
			return;
		};
		
		if (action == 'cancel') {
			IKLAD_2423.send_form_data(form_id, msg_id, 'action=cancel');
			return;
		};
		
		if (action == 'close_chat') {
			IKLAD_TEMPLATE_2423.close_chat_window();
			return;
		};
		
	},
	
	__QF_blink_field_error: function (in_iframe, el_id, el_class, flag) {
		var el
		
		if (in_iframe) {
			el = IKLAD_TEMPLATE_2423.msgs_ifr.document.getElementById(el_id);
		} else {
			el = document.getElementById(el_id);
		};
		
		if (! el) return;

		
		if (IKLAD_TEMPLATE_2423.__QF_blink_field_error_count-- <= 0) {
			IKLAD_TEMPLATE_2423.__QF_blink_field_error_count = 7; //reset
			el.className = el.className.replace(el_class, '');
			return;
		};
		
		if (flag) {
			el.className += ' ' + el_class;
		} else {
			el.className = el.className.replace(el_class, '');
		};
		
		var code = 'IKLAD_TEMPLATE_2423.__QF_blink_field_error(' + (in_iframe ? 'true' : 'false') + ',\'' + el_id + '\',\'' + el_class + '\',' + (flag ? 'false' : 'true') + ')';
		setTimeout(code, 100);
	},
	
	
	__QF_show_form: function (display, html, msg_id) {
		if (display == 'modal') {
			document.getElementById('MODAL_FORM_LAYER_CONTAINER_2423').innerHTML = html;
			IKLAD_TEMPLATE_2423.__show_layer('modal_form');
		};
		
		if (display == 'top') {
			IKLAD_TEMPLATE_2423.top_form_container.innerHTML = html;
			IKLAD_TEMPLATE_2423.top_form_container_tr.style.display = '';
		};
		
		if (display == 'bottom') {
			IKLAD_TEMPLATE_2423.bottom_form_container.innerHTML = html;
			IKLAD_TEMPLATE_2423.bottom_form_container_tr.style.display = '';
		};
		
		if (display == 'inchat') {
			var el = IKLAD_TEMPLATE_2423.msgs_ifr.document.getElementById('msg_' + msg_id);
			if (el) {
				el.innerHTML = html;
				el.style.display = '';
			};
			
			IKLAD_TEMPLATE_2423.scrol_msg_list_to_bottom();
		};
		
			try {
				    IKLAD_TEMPLATE_2423.scrol_msg_list_to_bottom();
			} catch (e) {};
		
	},
	
	__QF_hide_form: function (display, form_id, msg_id) {
		if (display == 'modal') {
			IKLAD_TEMPLATE_2423.__show_layer('default');
		};
		
		if (display == 'top') {
			IKLAD_TEMPLATE_2423.top_form_container_tr.style.display = 'none';
		};
		
		if (display == 'bottom') {
			IKLAD_TEMPLATE_2423.bottom_form_container_tr.style.display = 'none';
		};
		
		if (display == 'inchat') {
			var el = IKLAD_TEMPLATE_2423.msgs_ifr.document.getElementById('msg_' + msg_id);
			if (el) el.style.display = 'none';
		};
		
		try {
			    IKLAD_TEMPLATE_2423.scrol_msg_list_to_bottom();
		} catch (e) {};

		
	},
	
	
	
	
	
	//--------------------------------------------------------------------------------
	//--------------------------------------------------------------------------------
	//--------------------------------------------------------------------------------
	
	//LOCAL template functions/variables
	
	original_form_height: '',
	msgs_ifr: '',	//iframe variable holder
	msgs_container: '',	//msgs_container in iframe
	top_form_container: '',
	top_form_container_tr: '',
	bottom_form_container: '',
	bottom_form_container_tr: '',
	msgs_count: 0,
	operator_typing_container : '',
	operator_typing_msg_container: '',
	sound_enabled: true,
	chat_window_open: false,
	
	
	
	local_sound_on_off: function () {
		IKLAD_TEMPLATE_2423.sound_enabled = ! IKLAD_TEMPLATE_2423.sound_enabled;
		
		var img = document.getElementById('SOUND_ON_OFF_IMG_2423');
		
		if (IKLAD_TEMPLATE_2423.sound_enabled) {
			img.title = 'Звук включен';
			img.src = 'data:image/png;base64,';
		} else {
			img.title = 'Звук выключен';
			img.src = 'data:image/png;base64,';
		};
	},
	
	
	
	
	
	
	
	
	
	
	
	
	local_on_chat_message: function (msg_obj, play_sound) {
		var e = IKLAD_TEMPLATE_2423.msgs_ifr.document.createElement('div');
		e.className = 'msg_line';
		e.id = 'msg_' + msg_obj.id;
		
		if (msg_obj.type == 'msg') {
			var line = '<table width=100% cellpadding=0 cellspacing=0>';
			
			var tm = '';
			if (msg_obj.dt.match(/ (\d\d?:\d\d):\d\d/)) {
				tm = RegExp.$1;
			};
			
			if (msg_obj.to == 'client') {
				
				//if (msg_obj.show_operator_name == '1') {
				//	line += msg_obj.operator_name + ':';
				//};
				
				line += '<tr>';
				line += '<td class="msg_from_operator_left"></td>';
				line += '<td class="msg_from_operator"><div class="msg_from_operator">' + msg_obj.text + '</div></td>';
				line += '<td class="msg_from_operator_time">' + tm + '</td>';
				line += '</tr>';
			} else {
				
				line += '<tr>';
				line += '<td class="msg_from_client_time">' + tm + '</td>';
				line += '<td class="msg_from_client"><div class="msg_from_client">' + msg_obj.text + '</div></td>';
				line += '<td class="msg_from_client_right"></td>';
				line += '</tr>';
			};
			
			line += '</table>';
			
			
			e.innerHTML = line;
		};
		
		if (msg_obj.type == 'form') {
			e.className += ' msg_line_form';
			e.style.display = 'none';
			if (msg_obj.completed == 1) {
				// do no show anything
			} else {
				IKLAD_2423.get_form_data(msg_obj.text, msg_obj.id);		// form_data  will be called as result
			};
		};
		
		IKLAD_TEMPLATE_2423.msgs_container.appendChild(e);
		IKLAD_TEMPLATE_2423.msgs_count++
		IKLAD_TEMPLATE_2423.scrol_msg_list_to_bottom();
		
		if (IKLAD_TEMPLATE_2423.sound_enabled && IKLAD_TEMPLATE_2423.chat_window_open && play_sound) {
			IKLAD_2423.play_sound('new_message');
		};
	},
	
	scrol_msg_list_to_bottom: function () {
		var last_elem = IKLAD_TEMPLATE_2423.msgs_container.lastChild;
		
		if (IKLAD_TEMPLATE_2423.operator_typing_container.style.display != 'none') {
			last_elem = IKLAD_TEMPLATE_2423.operator_typing_msg_container;
		};
		
		if (last_elem) {
			var pos = IKLAD_FUNC.find_object_position(last_elem);
			IKLAD_TEMPLATE_2423.msgs_ifr.scrollTo(0, pos[1]);
		};
	},
	
	open_in_new_window: function () {
		var w = parseInt(document.getElementById('IKLAD_FLOAT_FORM_DIV_2423').offsetWidth);
		var h = parseInt(document.getElementById('IKLAD_FLOAT_FORM_DIV_2423').offsetHeight);
		
		IKLAD_TEMPLATE_2423.close_chat_window();
		IKLAD_2423.open_in_new_window({
			'width': w,
			'height': h
		});
	},
	

	draggable_initilized: false,	

	tmpl_open_window: function () {
		
		
		
		
		
		if (typeof IKLAD_TEMPLATE_2423.animate_window_open == 'function') {
		 IKLAD_TEMPLATE_2423.animate_window_open();
		} else {
			
		 		IKLAD_TEMPLATE_2423.std_window_open();
		 	
		};
		
		
		//this is required for IE
		if (navigator.appName.indexOf("Internet Explorer") != -1) {
			var parent = document.getElementById('IKLAD_MSGS_IFR_PARENT_2423');
			document.getElementById('IKLAD_MSGS_IFR_2423').height = parent.offsetHeight + 'px';
			
			parent = document.getElementById('IKLAD_TEXTAREA_ROW_2423');
			document.getElementById('IKLAD_TEXTAREA_2423').style.height = parseInt(parent.offsetHeight) + 'px';
		};
	},
	
	
	tmpl_close_window: function () {
		if (typeof IKLAD_TEMPLATE_2423.animate_window_close == 'function') {
		 IKLAD_TEMPLATE_2423.animate_window_close();
		} else {
		 IKLAD_TEMPLATE_2423.std_window_close();
		};
	},
	
	
	std_window_open: function (options) {
		var form_div = document.getElementById('IKLAD_FLOAT_FORM_DIV_2423');
		var new_style_display = '';
		
		try {
			if (typeof options == 'object') {
				var units = 'px';
				
				if (options.left) { 	
					if (options.left.match(/(px|%|em)$/)) { units = RegExp.$1; } else { units = 'px'; };
					form_div.style.left = parseInt(options.left) + units;
				};
				if (options.right) {
					if (options.right.match(/(px|%|em)$/)) { units = RegExp.$1; } else { units = 'px'; };
					form_div.style.right = parseInt(options.right) + units;
				};
				if (options.top) {
					if (options.top.match(/(px|%|em)$/)) { units = RegExp.$1; } else { units = 'px'; };
					form_div.style.top = parseInt(options.top) + units;
				};
				if (options.bottom) {
					if (options.bottom.match(/(px|%|em)$/)) { units = RegExp.$1; } else { units = 'px'; };
					form_div.style.bottom = parseInt(options.bottom) + units;
				};
				if (options.height) {
					if (options.height.match(/(px|%|em)$/)) { units = RegExp.$1; } else { units = 'px'; };
					form_div.style.height = parseInt(options.height) + units;
				};
				if (options.width) {
					if (options.width.match(/(px|%|em)$/)) { units = RegExp.$1; } else { units = 'px'; };
					form_div.style.width = parseInt(options.width) + units;
				};
				if (typeof(options.display) != 'undefined') {
					new_style_display = options.display;
				};
			};
		} catch (e) {};
		
		form_div.style.display = new_style_display;
	},
	
	std_window_close: function () {
		document.getElementById('IKLAD_FLOAT_FORM_DIV_2423').style.display = 'none';
	},
	

	
	
	
	
	animate_window_open: function () {
		if (typeof IKLAD_TEMPLATE_2423.get_chat_form_width_height == 'function') {
		 IKLAD_TEMPLATE_2423.get_chat_form_width_height();
		};
		
		
		
		
		  
			
			
		
		  
		     try {
		     	var btm = IKLAD_TEMPLATE_2423.form_height + parseInt($('#IKLAD_FLOAT_FORM_DIV_2423').css('margin-bottom'));
		      IKLAD_TEMPLATE_2423.std_window_open( { bottom: -1*btm + 'px' } );
		      $("#IKLAD_FLOAT_FORM_DIV_2423").stop(true).css({ 'bottom': -1*btm + 'px' }).animate({ "bottom": "0px" }, 600 );
		     } catch (e) {
		      IKLAD_TEMPLATE_2423.std_window_open( { bottom: '0px' } );
		     };
			
		
		  

		
		
	
	},

	animate_window_close: function () {
		try {
		 
		 
		  
			
			
		
		  
		  	 var btm = IKLAD_TEMPLATE_2423.form_height + parseInt($('#IKLAD_FLOAT_FORM_DIV_2423').css('margin-bottom'));
	       $("#IKLAD_FLOAT_FORM_DIV_2423").stop(true).animate({ "bottom": -1*btm + "px" }, 600, function(){
	          IKLAD_TEMPLATE_2423.std_window_close();
	          $("#IKLAD_FLOAT_FORM_DIV_2423").css({"bottom": "0px"});  //keep it back
	       });
			
		
		  
			
		
		 
		
		 } catch (e) {
		  IKLAD_TEMPLATE_2423.std_window_close();
		 };
	},
	
	
	

	
	
	
	form_height: 0,
	form_width: 0,
	form_width_height_set: false,
	
	get_chat_form_width_height: function () {
		if (IKLAD_TEMPLATE_2423.form_width_height_set) return;
		
		IKLAD_TEMPLATE_2423.form_width_height_set = true;
		
		var float_form = document.getElementById('IKLAD_FLOAT_FORM_DIV_2423');
		
	  if (float_form.style.display == '') {
			IKLAD_TEMPLATE_2423.form_height = float_form.offsetHeight;
			IKLAD_TEMPLATE_2423.form_width = float_form.offsetWidth;
	  } else {
	
	   var old_left = float_form.style.left;
	   var old_right = float_form.style.right;
	
	   if (old_left) {
	     float_form.style.left = '-10000px';
	     float_form.style.display = '';
	     IKLAD_TEMPLATE_2423.form_height = float_form.offsetHeight;
	     IKLAD_TEMPLATE_2423.form_width = float_form.offsetWidth;
	     float_form.style.display = 'none';
	     float_form.style.left = old_left;
	   };
	
	   if (old_right) {
	     float_form.style.right = '-10000px';
	     float_form.style.display = '';
	     IKLAD_TEMPLATE_2423.form_height = float_form.offsetHeight;
	     IKLAD_TEMPLATE_2423.form_width = float_form.offsetWidth;
	     float_form.style.display = 'none';
	     float_form.style.right = old_right;
	   };
	
	  };
	},
	
	
	
	
	__form_el_focus: function (el, def_text) {		
		if (el.value == def_text) {
			el.value = '';
			el.style.color = 'black';
		};
	},
	
	__form_el_blur: function (el, def_text) {
		if (el.value == '') {
			el.value = def_text;
			el.style.color = 'gray';
		};
	},
	
	
	textarea_prev_color: '',
	
	textarea_focus: function () {
		var t = document.getElementById('IKLAD_TEXTAREA_2423');
		
		if (t.value == 'Напишите ваше сообщение тут ...') {
			t.value = '';
			IKLAD_TEMPLATE_2423.textarea_prev_color = t.style.color;
			t.style.color = 'black';
		};
	},
	
	textarea_blur: function () {
		var t = document.getElementById('IKLAD_TEXTAREA_2423');
		
		if (t.value == '') {
			t.value = 'Напишите ваше сообщение тут ...';
			t.style.color = IKLAD_TEMPLATE_2423.textarea_prev_color;
		};
	},
	
	textarea_onkeydown: function (e) {
		if ((e.ctrlKey || e.altKey) && e.keyCode == 13) {
			document.getElementById('IKLAD_TEXTAREA_2423').value += "\n";
			try { if (e) e.preventDefault();} catch (er) {};
			return false;
		};
		
		if (e.keyCode == 13) {	// enter (w/o ctrl)
			IKLAD_TEMPLATE_2423.local_send_message();
			try { if (e) e.preventDefault();} catch (er) {};
			return false;
		};
		
		return true;
	},
	
	textarea_onkeypress: function () {
		IKLAD_TEMPLATE_2423.track_keyboard_activity();
	},
	
	local_send_message: function () {
		var t = document.getElementById('IKLAD_TEXTAREA_2423');
		
		if (t.value == '') return; //do not send empty msg
		if (t.value == 'Напишите ваше сообщение тут ...') return;	//do not send def text by clicking 'send btn'
		
		IKLAD_2423.send_message({'text': t.value});
		t.value = '';
		try { t.focus(); } catch (e) {};
	},
	

	
	//keyboard activity tracking
	keyboard_active: false,
	keyboard_activity_timeout_timer: '',
	last_send_keyboard_status: '',
	
	track_keyboard_activity: function () {
		var current_text = document.getElementById('IKLAD_TEXTAREA_2423').value;
		
		//obj.type == 'client_typing'			- operator keyboard activity
		//obj.status = 'composing' || 'paused' || ''				('' = clear status)
		
		if ((current_text == '') || (current_text == 'Напишите ваше сообщение тут ...')) {
			
			if (IKLAD_TEMPLATE_2423.keyboard_active || (IKLAD_2423.last_send_keyboard_status != '')) {
				IKLAD_2423.notify_client_activity({'type': 'client_typing', 'status': ''});
				IKLAD_2423.last_send_keyboard_status = '';
				IKLAD_TEMPLATE_2423.keyboard_active = false;
				
				clearTimeout(IKLAD_TEMPLATE_2423.keyboard_activity_timeout_timer);
				IKLAD_TEMPLATE_2423.keyboard_activity_timeout_timer = null;
			};
			
		} else {
			
			if (IKLAD_TEMPLATE_2423.keyboard_active) {
				//restart 'pause timeout' timer
				clearTimeout(IKLAD_TEMPLATE_2423.keyboard_activity_timeout_timer);
				IKLAD_TEMPLATE_2423.keyboard_activity_timeout_timer = setTimeout(IKLAD_TEMPLATE_2423.track_keyboard_activity_timeout, 5000);
			} else {
				//start
				IKLAD_2423.notify_client_activity({'type': 'client_typing', 'status': 'composing'});
				IKLAD_2423.last_send_keyboard_status = 'composing';
				IKLAD_TEMPLATE_2423.keyboard_active = true;
				IKLAD_TEMPLATE_2423.keyboard_activity_timeout_timer = setTimeout(IKLAD_TEMPLATE_2423.track_keyboard_activity_timeout, 5000);
			};
			
		};
	},
	
	track_keyboard_activity_timeout: function () {
		var current_text = document.getElementById('IKLAD_TEXTAREA_2423').value;
		
		clearTimeout(IKLAD_TEMPLATE_2423.keyboard_activity_timeout_timer);
		IKLAD_TEMPLATE_2423.keyboard_active = false;
		
		if ((current_text == '') || (current_text == 'Напишите ваше сообщение тут ...')) {
			IKLAD_2423.notify_client_activity({'type': 'client_typing', 'status': ''});
			IKLAD_2423.last_send_keyboard_status = '';
		} else {
			IKLAD_2423.notify_client_activity({'type': 'client_typing', 'status': 'paused'});
			IKLAD_2423.last_send_keyboard_status = 'paused';
		};
	}
	
};

		IKLAD_2423.TEMPLATE.HTML_CODE += '<style type="text/css" id="IKLAD_CSS_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#IKLAD_FLOAT_FORM_DIV_2423, #IKLAD_FLOAT_FORM_DIV_2423 *, #IKLAD_OPINION_BOX_2423, #IKLAD_OPINION_BOX_2423 * {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  padding: 0;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  margin: 0;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	border-spacing: 0;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	border-collapse: collapse;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	border: 0 none;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  font-size: 11px;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  line-height: normal !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  vertical-align: top;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	text-align: left;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	text-indent: 0;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	text-decoration: none;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	outline: none;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	width: auto;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	min-width: 0;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	max-width: none;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	height: auto;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	min-height: 0;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	max-height: none;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	background: none;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	box-sizing: border-box;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	font-family: Verdana;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	color: black;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#IKLAD_FLOAT_FORM_DIV_2423 img {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	display: inline;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#IKLAD_TEXTAREA_2423 {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	resize: none; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	overflow: auto; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	width: 100% !important; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	height: 100% !important; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	border: 0; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	padding: 3px 0 0 4px !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	outline: 0;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	font-size: 11px !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	box-shadow: none !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	font-weight: normal;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  background-color: rgba(0,0,0,0);';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  box-sizing: border-box;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#IKLAD_TEXTAREA_ROW_2423 {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  background: -webkit-linear-gradient(top, #f1f6f9, #ffffff);';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  background: -o-linear-gradient(bottom, #f1f6f9, #ffffff);';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  background: -moz-linear-gradient(bottom, #f1f6f9, #ffffff);';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  background: linear-gradient(to bottom, #f1f6f9, #ffffff);';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#MODAL_FORM_LAYER_2423, #LOADING_LAYER_2423, #ONLINE_LAYER_2423 {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	background-color: #ecf3f8;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#CHAT_FORM_TITLE_2423, #FORM_2423 .FORM_HEADER_2423 h1  {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	padding: 4px; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	font-weight: bold; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	font-size: 14px; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	color: gray;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#FORM_2423 .FORM_TITLE_2423 h3 {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	padding: 4px; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	color: #7a7a7a; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	text-align: left; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	min-height: 50px;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	font-weight: normal; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#FORM_2423 .FORM_FIELD_TITLE_2423 > td {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	color: black;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	padding: 3px 4px;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#FORM_2423 .FORM_FIELD_2423 > td {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	padding: 2px 0;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#FORM_2423 .FORM_FIELD_2423 input {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	resize: none; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	border: 1px solid #EBF1F8;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	outline: 0;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	width: 100%;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	font-size: 11px !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	padding-left: 4px !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	box-shadow: none !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	font-weight: normal !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	height: 23px !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	background-color: white;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  background: linear-gradient(to bottom, #f1f6f9, #ffffff) repeat scroll 0 0 rgba(0, 0, 0, 0);';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#FORM_2423 .FORM_FIELD_2423 input.FORM_INPUT_ERROR_2423 {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	background: #77C7FF;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#FORM_2423 .FORM_FIELD_2423 textarea {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	resize: none; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	overflow: auto; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	width: 100% !important; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	height: 100% !important; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	border: 0; ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	padding: 3px 0 0 4px !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	outline: 0;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	font-size: 11px !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	box-shadow: none !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	font-weight: normal;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  background: white;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  border: 1px solid #EBF1F8;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  box-sizing: border-box;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#FORM_2423 .FORM_FIELD_2423 textarea.FORM_INPUT_ERROR_2423 {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	background: #77C7FF;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#FORM_2423 .FORM_BUTTON_CONTAINER_2423 > td {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	padding: 2px 0 4px 0;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	text-align: center;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#FORM_2423 .FORM_BUTTON_CONTAINER_2423 button {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	color: black;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	border: 1px solid #EBF1F8;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	text-decoration: none;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	font-weight: bold;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	font-size: 12px;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	cursor: pointer;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	height: 23px !important;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	outline: none;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	padding: 0 2px;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#FORM_2423 .FORM_BUTTON_CONTAINER_2423 button.FORM_BUTTON_DISABLED_2423 {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	color: gray;';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '#IKLAD_HEADER_TD_2423 {';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  background: -webkit-linear-gradient(top, #feffff, #f7fafc);';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  background: -o-linear-gradient(bottom, #feffff, #f7fafc);';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  background: -moz-linear-gradient(bottom, #feffff, #f7fafc);';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '  background: linear-gradient(to bottom, #feffff, #f7fafc);';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '}';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '</style>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '<table style="width:300px; display:none; border: 1px solid #77c7ff; border-radius: 4px; z-index:2100000010; box-shadow: 0px 0px 7px 0px rgba(0, 0, 0, 0.5);   position: fixed; right:0; bottom:0;  margin-right:5px; margin-bottom:0px;      " id="IKLAD_FLOAT_FORM_DIV_2423" valign=top>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	<tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '		<td style="padding-top: 2px; background: #feffff; height:350px;" id="IKLAD_SET_HEIGHT_TD_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '            ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '            <table style="height:100%; width:100%;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                <tr style="height: 63px;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                    <td style="border-bottom: 1px solid #c6d6e8; padding-top: 7px;" id="IKLAD_HEADER_TD_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                       <table style="width:100%; height: 100%;" >';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                               ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                           <tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                               <td style="width:50px; padding: 0 5px 0 5px; vertical-align: middle;" align=center valign=middle>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                               		<!--template must define [VAR_OPERATOR_PHOTO_MAX_LONGEST_SIDE] variable to automatically resize photo-->';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '																	';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                  <img style="" src="//chat.iklad-chat.biz/img/templates/design3/def_photo.png" id="IKLAD_OPERATOR_PHOTO_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                               </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                               ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                               <td style="width:100%; " valign=top>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                    <table style="height:100%; width:100%;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                        <tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            <td style="padding: 6px 0 0 5px; font-size: 14px;" align=left valign=top>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                                <table style="padding-top: 2px;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                                		<tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                                			<td colspan=2 style="padding-left: 1px; font-size: 14px; color:#3e99d2; font-weight: bold;" id="IKLAD_TITLE_TEXT_2423">&#1054;&#1087;&#1077;&#1088;&#1072;&#1090;&#1086;&#1088;</td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                                		</tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                                    <tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                                      <td style="width: 7px; padding: 4px 0 0 3px;"><img src="data:image/png;base64," id="IKLAD_OP_STATUS_IMG_2423"></td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                                      <td valign=top align=left style="padding-left: 5px; color: #92d23e; font-size: 11px;" id="IKLAD_OP_STATUS_TEXT_2423">онлайн</td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                                    </tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                                </table>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            <td align=right valign=top style="width:20px; padding: 0 10px 0 0;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            	<table align=right>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            		<tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            			';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            			<td style="padding: 0 3px 0 3px;" id="IKLAD_OPEN_CHAT_IN_NEW_WINDOW_ICON_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            				<a href="#" onclick="IKLAD_TEMPLATE_2423.open_in_new_window(); return false;" title="Открыть в новом окне"><img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAsAAAALCAIAAAAmzuBxAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPAAAAF5JREFUeNpivPvx//ZHDI+/MGACWR4GTzkGxpYz/z//ZsAFeFkZWCDSHRZYpCtOMABlWZD5yACuh4mBECCsggWrycg2sqA5DZ8ZWL0DUgH0MdBLWHVDwoORYJgCBBgAsiEhN60K284AAAAASUVORK5CYII="/></a>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            			</td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            			';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            			<td style="padding: 0 0 0 3px;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            				<a href="#" onclick="IKLAD_TEMPLATE_2423.close_chat_window(); return false;" title="Скрыть чат"><img src="data:image/png;base64,"/></a>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            			</td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            		</tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            	</table>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                               ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                               ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                            </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                        </tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                    </table>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                               </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                           </tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                       </table>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                    </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                </tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                <tr style="height:100%;" id="LOADING_LAYER_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                    <td valign=top style="padding: 0 10px;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	            				<table style="width: 100%; height: 100%;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	            					<tr style="height: 100%;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	            						<td style="text-align: center; vertical-align: middle;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	            							<img src="data:image/gif;base64," />';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	            						</td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	            					</tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	            				</table>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                    </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                </tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                <tr style="height:100%; display: none;" id="MODAL_FORM_LAYER_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                  <td valign=top style="height: 100%; padding: 0 8px;" id="MODAL_FORM_LAYER_CONTAINER_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                  </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                </tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                <tr style="height:100%; display: none;" id="ONLINE_LAYER_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                	<td valign=top>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                  	<table style="height: 100%; width: 100%;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                  		<tr id="TOP_FORM_CONTAINER_TR" style="display: none;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '			            			<td style="padding: 2px 5px; border-bottom: 1px solid #dee1df;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '			            				<div id="TOP_FORM_CONTAINER" style=""></div>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '			            			</td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '			            		</tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '			            		';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                  		<tr style="height:100%;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '		                    <td valign=top style="background: white;" id="IKLAD_MSGS_IFR_PARENT_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '		                    	<iframe name="IKLAD_MSGS_IFR_2423" id="IKLAD_MSGS_IFR_2423" frameborder="0" border="0" scrolling="auto" width="100%" height="100%" style="border:0px; padding: 0; margin: 0; overflow: auto; width: 100%; height: 100%;"></iframe>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '		                    </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                  		</tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '			                 ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '			            		<tr id="BOTTOM_FORM_CONTAINER_TR" style="display: none;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '			            			<td style="padding: 2px 8px; border-top: 1px solid #dee1df;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '			            				<div id="BOTTOM_FORM_CONTAINER"></div>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '			            			</td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '			            		</tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                  	</table>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                	</td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                </tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                <tr style="height: 40px; display: none;" id="IKLAD_TEXTAREA_ROW_CONTAINER_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                    <td style="border-top: 1px solid #c6d6e8; background: white;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                        <table style="height: 100%; width:100%;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                            <tr style="height: 100%;" id="IKLAD_TEXTAREA_ROW_2423"> ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                <td valign=top>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                	<textarea id="IKLAD_TEXTAREA_2423" style="color: gray;" onfocus="IKLAD_TEMPLATE_2423.textarea_focus();" onblur="IKLAD_TEMPLATE_2423.textarea_blur();" onkeydown="return IKLAD_TEMPLATE_2423.textarea_onkeydown(event);" onkeypress="IKLAD_TEMPLATE_2423.textarea_onkeypress();">Напишите ваше сообщение тут ...</textarea>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                <td style="width: 16px; opacity: 0.5; padding-right: 8px; vertical-align: middle;" align=center valign=middle id="IKLAD_SEND_BTN_CONTAINER_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                	<a href="#" onclick="IKLAD_TEMPLATE_2423.local_send_message(); return false;" title="Отправить сообщение"><img border=0 src="data:image/png;base64,"/></a>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                                </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                            </tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                        </table>      ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                    </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                </tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                <tr style="height: 24px; ">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                   <td style="border-top: 1px solid #c6d6e8; background-color: #77c7ff; vertical-align: middle;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                   ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                    <table style="width: 100%;">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                    	<tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                        <td style="padding-left: 5px; text-align: left;" valign=middle>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                        ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                        </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '												';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '					';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '												';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '					';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '												';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '									      ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '									      ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '									      <td style="width:20px; padding-left: 5px; text-align: center;" align=center id="IKLAD_SOUND_ON_OFF_ICON_2423">';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '									        <a href="#" onclick="IKLAD_TEMPLATE_2423.local_sound_on_off(); return false;"><img title="Звук включен" id="SOUND_ON_OFF_IMG_2423" border=0 src="data:image/png;base64,"></a>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '									      </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '									      ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '									      ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '									      <td style="width:5px;"></td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '									    </tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                    </table>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                    ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                   </td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '                </tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '            </table>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '            ';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '		</td>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '	</tr>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";IKLAD_2423.TEMPLATE.HTML_CODE += '</table>';
IKLAD_2423.TEMPLATE.HTML_CODE += "\n";
		
		
		
		IKLAD_2423.init();
	